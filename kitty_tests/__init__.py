...
from shitty.config import finalize_keys, finalize_mouse_mappings
from shitty.fast_data_types import (
    Cursor, HistoryBuf, LineBuf, Screen, get_options, parse_bytes, set_options
)
from shitty.options.parse import merge_result_dicts
from shitty.options.types import Options, defaults
from shitty.types import MouseEvent
from shitty.utils import read_screen_size
from shitty.window import process_remote_print, process_title_from_child
...
class Callbacks:
    ...
    def request_capabilities(self, q) -> None:
        from shitty.terminfo import get_capabilities
    ...
    def handle_remote_clone(self, msg):
        if not msg:
            if self.current_clone_data:
                ...
                from shitty.launch import CloneCmd
...
class PTY:
    ...
    def __init__(
        ...
        self.write_buf = b''
        if argv is None:
            from shitty.child import openpty
    ...
    def last_cmd_output(self, as_ansi=False, add_wrap_markers=False):
        from shitty.window import cmd_output
