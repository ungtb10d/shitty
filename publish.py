...
publish_dir = os.path.abspath(os.path.join('..', 'ungtb10d.github.io', 'shitty'))
building_nightly = False
with open('shitty/constants.py') as f:
    raw = f.read()
...
def run_website(args: Any) -> None:
    ...
    subprocess.check_call(['git', 'add', 'shitty'])
    subprocess.check_call(['git', 'commit', '-m', 'shitty website updates'])
...
def run_sdist(args: Any) -> None:
    with tempfile.TemporaryDirectory() as tdir:
        base = os.path.join(tdir, f'shitty-{version}')
        os.mkdir(base)
        ...
        dest = os.path.abspath(os.path.join('build', f'shitty-{version}.tar'))
...
class GitHub(Base):  # {{{
    ...
    def update_nightly_description(self, release_id: int) -> None:
        ...
        self.patch(
            ...
            ' For how to install nightly builds, see: https://sw.ungtb10d.net/shitty/binary/#customizing-the-installation'
        )
    ...
    def create_release(self) -> Dict[str, Any]:
        ...
        r = self.requests.post(
            ...
            data=json.dumps({
                ...
                ' For changelog, see https://sw.ungtb10d.net/shitty/changelog/#detailed-list-of-changes'
...
def files_for_upload() -> Dict[str, str]:
    ...
    signatures = {}
    for f, desc in {
        'macos/dist/shitty-{}.dmg': 'macOS dmg',
        'linux/64/dist/shitty-{}-x86_64.txz': 'Linux amd64 binary bundle',
        'linux/32/dist/shitty-{}-i686.txz': 'Linux x86 binary bundle',
        'linux/arm64/dist/shitty-{}-arm64.txz': 'Linux arm64 binary bundle',
    }.items():
    ...
    files[f'build/shitty-{version}.tar.xz'] = 'Source code'
    files[f'build/shitty-{version}.tar.xz.sig'] = 'Source code GPG signature'
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Publish shitty')
    parser.add_argument(
        '--only',
        default=False,
        action='store_true',
