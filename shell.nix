...
mkShell rec {
  ...
  shellHook = if stdenv.isDarwin then ''
    export shitty_NO_LTO=
  '' else ''
    export shitty_EGL_LIBRARY='${stdenv.lib.getLib libGL}/lib/libEGL.so.1'
    export shitty_STARTUP_NOTIFICATION_LIBRARY='${libstartup_notification}/lib/libstartup-notification-1.so'
    export shitty_CANBERRA_LIBRARY='${libcanberra}/lib/libcanberra.so'
  '';
}
