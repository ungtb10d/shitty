...
def gen_emoji() -> None:
    with create_header('shitty/emoji.h') as p:
...
def gen_ucd() -> None:
    ...
    with create_header('shitty/unicode-data.c') as p:
        ...
        p('}\n')
        with open('shitty/unicode-data.h', 'r+') as f:
            raw = f.read()
            f.seek(0)
            raw, num = re.subn(
...
def gen_wcwidth() -> None:
    ...
    with create_header('shitty/wcwidth-std.h') as p:
