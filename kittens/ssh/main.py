...
from shitty.shell_integration import as_str_literal
from shitty.shm import SharedMemory
    ...
    with tarfile.open(mode=f'w:{compression}', fileobj=buf, encoding='utf-8') as tf:
        ...
        if ksi:
            ...
            tf.add(shell_integration_dir, arcname=arcname, filter=filter_from_globs(
                ...
                f'{arcname}/zsh/shitty.zsh',  # present for legacy compat not needed by ssh kitten
            ))
        if ssh_opts.remote_kitty != 'no':
        ...
        tf.add(f'{terminfo_dir}/shitty.terminfo', arcname='home/.terminfo/shitty.terminfo', filter=normalize_tarinfo)
        tf.add(glob.glob(f'{terminfo_dir}/*/xterm-shitty')[0], arcname='home/.terminfo/x/xterm-shitty', filter=normalize_tarinfo)
    return buf.getvalue()
...
def dcs_to_kitty(payload: Union[bytes, str], type: str = 'ssh') -> bytes:
    ...
    ans = b'\033P@shitty-' + type.encode('ascii') + b'|' + payload
...
def main(args: List[str]) -> None:
    ...
    if not os.environ.get('KITTY_WINDOW_ID') or not os.environ.get('KITTY_PID'):
        raise SystemExit('The SSH kitten is meant to run inside a shitty window')
