...
set_xoptions(const char *exe_dir_c, const char *lc_ctype, bool from_source) {
    ...
    if (from_source) PySys_AddXOption(L"shitty_from_source=1");
    if (lc_ctype) {
...
run_embedded(const RunData run_data) {
    ...
    bypy_initialize_interpreter(L"shitty", python_home, L"shitty_main", extensions_dir, run_data.argc, run_data.argv);
    if (!set_xoptions(run_data.exe_dir, run_data.lc_ctype, false)) return 1;
    set_sys_bool("frozen", true);
    set_sys_string("shitty_extensions_dir", extensions_dir);
...
read_exe_path(char *exe, size_t buf_sz) {
    ...
    while (token != NULL) {
        char q[PATH_MAX + 1] = {0};
        snprintf(q, PATH_MAX, "%s/shitty", token);
        if (safe_realpath(q, exe, buf_sz)) return true;
        token = strtok(NULL, ":");
    }
    fprintf(stderr, "shitty not found in PATH aborting\n");
    return false;
}
...
int main(int argc, char *argv[]) {
    ...
    // Always use UTF-8 mode, see https://github.com/ungtb10d/shitty/issues/924
    Py_UTF8Mode = 1;
#endif
