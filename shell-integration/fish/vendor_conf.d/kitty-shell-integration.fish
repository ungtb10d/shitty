...
# "Warning: Update fish to version 3.3.0+ to enable shitty shell integration.\n"
set -q fish_killring || set -q status_generation || string match -qnv "3.1.*" "$version"
or echo -en \eP@shitty-print\|V2FybmluZzogVXBkYXRlIGZpc2ggdG8gdmVyc2lvbiAzLjMuMCsgdG8gZW5hYmxlIGtpdHR5IHNoZWxsIGludGVncmF0aW9uLgo=\e\\ && exit 0 || exit 0
...
function __ksi_schedule --on-event fish_prompt -d "Setup shitty integration after other scripts have run, we hope"
    ...
    if not contains "no-prompt-mark" $_ksi
        ...
        # With prompt marking, shitty clears the current prompt on resize,
    ...
    if not contains "no-cwd" $_ksi
        # This function name is from fish and will override the builtin one, which is enabled by default for shitty in fish 3.5.0+.
        ...
        function __update_cwd_osc --on-variable PWD -d "Report PWD changes to shitty"
            status is-command-substitution
            or echo -en "\e]7;shitty-shell-cwd://$hostname$PWD\a"
        end
        __update_cwd_osc
    end
...
function __ksi_transmit_data -d "Transmit data to shitty using chunked DCS escapes"
    set --local data_len (string length -- "$argv[1]")
    set --local pos 1
    set --local chunk_num 0
    while test "$pos" -le $data_len
        printf \eP@shitty-%s\|%s:%s\e\\ "$argv[2]" "$chunk_num" (string sub --start $pos --length 2048 -- $argv[1] | string collect)
        set pos (math $pos + 2048)
        set chunk_num (math $chunk_num + 1)
    end
    printf \eP@shitty-%s\|\e\\ "$argv[2]"
end

function clone-in-shitty -d "Clone the current fish session into a new shitty window"
    set --local data
    for a in $argv
        if contains -- "$a" -h --help
            echo "Clone the current fish session into a new shitty window."
            echo
            echo "For usage instructions see: https://sw.ungtb10d.net/shitty/shell-integration/#clone-shell"
            return
        end
...
function edit-in-shitty -d "Edit the specified file in a new shitty overlay using your preferred editor, even over SSH"
    set --local data
    set --local ed_filename ""
    set --local usage "Usage: edit-in-shitty [OPTIONS] FILE"
    for a in $argv
        if contains -- "$a" -h --help
            echo "$usage"
            echo
            echo "Edit the specified file in a shitty overlay window. Works over SSH as well."
            echo
            echo "For usage instructions see: https://sw.ungtb10d.net/shitty/shell-integration/#edit-file"
            return
        end
        set --local ea (printf "%s" "$a" | base64)
        set --append data "a=$ea"
