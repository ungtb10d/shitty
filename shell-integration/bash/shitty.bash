...
_ksi_main() {
    ...
    _ksi_debug_print() {
        ...
        builtin printf "\eP@shitty-print|%s\e\\" "${b//[[:space:]]}}"
    }
    ...
    _ksi_prompt_command() {
        ...
        if [[ -n "${_ksi_prompt[ps1]}" ]]; then
            if [[ "${_ksi_prompt[mark]}" == "y" && ( "${PS1}" == *"\n"* || "${PS1}" == *$'\n'* ) ]]; then
                ...
                # lines before the last line will be erased by shitty.
        ...
        if [[ "${_ksi_prompt[cwd]}" == "y" ]]; then
            ...
            if [[ "${_ksi_prompt[last_reported_cwd]}" != "$PWD" ]]; then
                ...
                builtin printf "\e]7;shitty-shell-cwd://%s%s\a" "$HOSTNAME" "$PWD"
            fi
        fi
    }
    ...
    if [[ "${_ksi_prompt[complete]}" == "y" ]]; then
        _ksi_completions() {
            ...
            src=$(builtin printf "%s\n" "${COMP_WORDS[@]:0:$limit}" | builtin command shitty +complete bash)
            if [[ $? == 0 ]]; then
                builtin eval "${src}"
            fi
        }
        builtin complete -o nospace -F _ksi_completions shitty
        builtin complete -o nospace -F _ksi_completions edit-in-shitty
        builtin complete -o nospace -F _ksi_completions clone-in-shitty
    fi
...
_ksi_transmit_data() {
    ...
    while [ $pos -lt ${#data} ]; do
        ...
        pos=$(($pos+2048))
        builtin printf '\eP@shitty-%s|%s:%s\e\\' "${2}" "${chunk_num}" "${chunk}"
        chunk_num=$(($chunk_num+1))
    done
    # save history so it is available in new shell
    [ "$3" = "save_history" ] && builtin history -a
    builtin printf '\eP@shitty-%s|\e\\' "${2}"
}
...
clone-in-shitty() {
    ...
    while :; do
        case "$1" in
            "") break;;
            -h|--help)
                builtin printf "%s\n\n%s\n" "Clone the current bash session into a new shitty window." "For usage instructions see: https://sw.ungtb10d.net/shitty/shell-integration/#clone-shell"
                return
                ;;
...
edit-in-shitty() {
    ...
    builtin local usage="Usage: edit-in-shitty [OPTIONS] FILE"
    data="cwd=$(builtin printf "%s" "$PWD" | builtin command base64)"
    while :; do
        case "$1" in
            "") break;;
            -h|--help)
                builtin printf "%s\n\n%s\n\n%s\n" "$usage" "Edit the specified file in a shitty overlay window. Works over SSH as well." "For usage instructions see: https://sw.ungtb10d.net/shitty/shell-integration/#edit-file"
                return
                ;;
