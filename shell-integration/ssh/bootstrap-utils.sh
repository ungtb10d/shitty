...
compile_terminfo() {
    ...
    if [ ! -f "$1/$tname/78/xterm-shitty" ]; then
        command mkdir -p "$1/$tname/78"
        command ln -sf "../x/xterm-shitty" "$1/$tname/78/xterm-shitty"
    fi

    if [ -e "/usr/share/misc/terminfo.cdb" ]; then
        # NetBSD requires this file, see https://github.com/ungtb10d/shitty/issues/4622
        command ln -sf "../../.terminfo.cdb" "$1/$tname/x/xterm-shitty"
        tname=".terminfo.cdb"
    fi
    ...
    if [ -x "$(command -v tic)" ]; then
        tic_out=$(command tic -x -o "$1/$tname" "$1/.terminfo/shitty.terminfo" 2>&1)
