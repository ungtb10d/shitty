...
# Use try-always to have the right error code.
{
    ...
    #
    # Source the user's zshenv before sourcing shitty.zsh because the former
    # might set fpath and other things without which shitty.zsh won't work.
    #
    ...
    if [[ -o 'interactive' && -n "${KITTY_SHELL_INTEGRATION-}" ]]; then
        ...
        'builtin' 'typeset' _ksi_file="${${(%):-%x}:A:h}"/shitty-integration
        if [[ -r "$_ksi_file" ]]; then
