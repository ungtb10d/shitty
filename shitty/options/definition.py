...
definition = Definition(
    ...
    Action('map', 'parse_map', {'keymap': 'KeyMap', 'sequence_map': 'SequenceMap'},
           ['KeyDefinition', 'shitty.conf.utils.KeyAction', 'shitty.types.SingleKey']),
    Action('mouse_map', 'parse_mouse_map', {'mousemap': 'MouseMap'}, ['MouseMapping', 'shitty.conf.utils.KeyAction']),
...
# fonts {{{
agr('fonts', 'Fonts', '''
shitty has very powerful font management. You can configure individual font faces
...
To get a full list of supported fonts use the `shitty list-fonts` command.
...
opt('force_ltr', 'no',
    ...
    long_text='''
shitty does not support BIDI (bidirectional text), however, for RTL scripts,
words are automatically displayed in RTL. That is to say, in an RTL script, the
words "HELLO WORLD" display in shitty as "WORLD HELLO", and if you try to select
...
writes into the selection buffer the character י.  shitty's default behavior is
...
will force shitty to always treat the text as LTR, which FriBidi expects for
terminals.
'''
    )
...
opt('adjust_line_height', '0',
    ...
    long_text='''
Change the size of each character cell shitty renders. You can use either
...
always render them.  You can tell shitty to not render them when the cursor is
over them by using :code:`cursor` to make editing easier, or have shitty never
render them at all by using :code:`always`, if you don't like them. The ligature
strategy can be set per-window either using the shitty remote control facility
or by defining shortcuts for it in shitty.conf, for example::
...
To get the PostScript name for a font, use :code:`shitty + list-fonts --psnames`:

.. code-block:: sh

    $ shitty + list-fonts --psnames | grep Fira
    Fira Code
...
copy to a private shitty buffer instead. Map a shortcut with the
...
You can run shitty with the :option:`shitty --debug-input` command line option
...
the entire screen on each loop, because shitty is so fast that partial screen
...
opt('remember_window_size', 'yes',
    option_type='to_bool',
    long_text='''
If enabled, the window size will be remembered so that new instances of shitty
...
opt('resize_draw_strategy', 'static',
    ...
    long_text='''
Choose how shitty draws a window while a resize is in progress. A value of
...
number of shitty windows in it. A value of zero disables confirmation. This
...
shitty config and not use a background color in the editor color scheme. Or use
...
opt('editor', '.',
    long_text='''
The console editor to use when editing the shitty config file or similar tasks. A
...
startup scripts but system-wide, otherwise shitty will not see it.
'''
    )
...
opt('allow_remote_control', 'no',
    ...
    long_text='''
Allow other programs to control shitty. If you turn this on other programs can
control all aspects of shitty, including sending text to shitty windows, opening
...
running within shitty to control it, with :code:`yes` or only programs that
connect to the socket specified with the :option:`shitty --listen-on` command
...
controlling shitty. Changing this option by reloading the config will only affect
...
opt('listen_on', 'none',
    long_text='''
Tell shitty to listen to the specified unix/tcp socket for remote control
connections. Note that this will apply to all shitty instances. It can be
overridden by the :option:`shitty --listen-on` command line flag. This option
accepts only UNIX sockets, such as unix:${TEMP}/myshitty or (on Linux)
unix:@myshitty. Environment variables are expanded. If {shitty_pid} is present
then it is replaced by the PID of the shitty process, otherwise the PID of the
shitty process is appended to the value, with a hyphen. This option is ignored
unless you also set :opt:`allow_remote_control` to enable remote control. See
the help for :option:`shitty --listen-on` for more details. Changing this option
...
opt('update_check_interval', '24',
    option_type='float',
    long_text='''
Periodically check if an update to shitty is available. If an update is found a
...
opt('startup_session', 'none',
    ...
    long_text='''
Path to a session file to use for all shitty instances. Can be overridden by
using the :option:`shitty --session` command line option for individual
instances. See :ref:`sessions` in the shitty documentation for details. Note that
relative paths are interpreted with respect to the shitty config directory.
...
opt('clipboard_control', 'write-clipboard write-primary',
    ...
    long_text='''
Allow programs running in shitty to read and write from the clipboard. You can
...
specify no-append to disable shitty's protocol extension for clipboard
...
``ask`` means that shitty will ask before opening the link.
'''
    )

opt('term', 'xterm-shitty',
    long_text='''
...
opt('wayland_titlebar_color', 'system',
    ...
    long_text='''
Change the color of the shitty window's titlebar on Wayland systems with client
...
opt('macos_titlebar_color', 'system',
    option_type='macos_titlebar_color',
    long_text='''
Change the color of the shitty window's titlebar on macOS. A value of
...
opt('macos_option_as_alt', 'no',
    ...
    long_text='''
Use the option key as an alt key. With this set to :code:`no`, shitty will use
...
opt('macos_hide_from_tasks', 'no',
    option_type='to_bool', ctype='bool',
    long_text='Hide the shitty window from running tasks (:kbd:`⌘+Tab`) on macOS.'
...
opt('macos_quit_when_last_window_closed', 'no',
    option_type='to_bool', ctype='bool',
    long_text='''
Have shitty quit when all the top-level windows are closed. By default, shitty
...
opt('macos_window_resizable', 'yes',
    option_type='to_bool', ctype='bool',
    long_text='''
Disable this if you want shitty top-level (OS) windows to not be resizable on
...
for a key, start shitty with the :option:`shitty --debug-input` option. Then shitty will
...
opt('shitty_mod', 'ctrl+shift',
    ...
    long_text='''
The value of :code:`shitty_mod` is used as the modifier for all default
shortcuts, you can change it in your shitty.conf to change the modifiers for all
...
opt('clear_all_shortcuts', 'no',
    ...
    long_text='''
You can have shitty remove all shortcut definition seen up to this point. Useful,
...
opt('+shitten_alias', 'hints hints --hints-offset=0',
    ...
    add_to_default=False,
    long_text='''
You can create aliases for shitten names, this allows overriding the defaults for
shitten options and can also be used to shorten repeated mappings of the same
shitten with a specific group of options. For example, the above alias changes
the default value of :option:`shitty +shitten hints --hints-offset` to zero for
...
You can open a new window that is allowed to control shitty via
the shitty remote control facility by prefixing the command line with @.
Any programs running in that window will be allowed to control shitty.
For example::
...
map('New OS window',
    ...
    long_text='''
Works like new_window above, except that it opens a top level OS shitty window.
...
You can change the font size for all top-level shitty OS windows at a time or
...
map('Open URL',
    'open_url shitty_mod+e shitten hints',
    long_text='''
...
shortcuts. For a full description see :doc:`shittens/hints`.
''')  # }}}
...
map('Unicode input',
    'input_unicode_character cmd+ctrl+space shitten unicode_input',
    only="macos",
    )
...
map('Open the shitty command shell',
    ...
    long_text='''
Open the shitty shell in a new window/tab/overlay/os_window to control shitty
using commands.
'''
    )
...
You can create shortcuts to clear/reset the terminal. For example::
    ...
    # Reset the terminal
    map shitty_mod+f9 clear_terminal reset active
    # Clear the terminal screen by erasing all contents
    map shitty_mod+f10 clear_terminal clear active
    # Clear the terminal scrollback by erasing it
    map shitty_mod+f11 clear_terminal scrollback active
    # Scroll the contents of the screen into the scrollback
    map shitty_mod+f12 clear_terminal scroll active
...
map('Reload shitty.conf',
    ...
    long_text='''
Reload shitty.conf, applying any changes since the last time it was loaded.
Note that a handful of settings cannot be dynamically changed and require a
full restart of shitty.  You can also map a keybinding to load a different
config file, for example::

    map f5 load_config /path/to/alternative/shitty.conf

Note that all setting from the original shitty.conf are discarded, in other words
...
map('Debug shitty configuration',
    ...
    long_text='''
Show details about exactly what configuration shitty is running with and
...
map('Send arbitrary text on key presses',
    ...
    add_to_default=False,
    long_text='''
You can tell shitty to send arbitrary (UTF-8) encoded text to
...
activate the shortcut. The possible values are :code:`normal` or :code:`application` or :code:`shitty`
...
terminals, and :code:`shitty` refers to the special shitty extended keyboard protocol.
