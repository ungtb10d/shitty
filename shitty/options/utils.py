...
import shitty.fast_data_types as defines
...
from shitty.constants import config_dir, is_macos
from shitty.fast_data_types import CURSOR_BEAM, CURSOR_BLOCK, CURSOR_UNDERLINE
from shitty.fonts import FontFeature
from shitty.key_names import (
    character_key_name_aliases, functional_key_name_aliases,
    get_key_name_lookup
)
from shitty.rgb import Color, color_as_int
from shitty.types import FloatEdges, MouseEvent, SingleKey
from shitty.utils import expandvars, log_error
...
mod_map = {'CTRL': 'CONTROL', 'CMD': 'SUPER', '⌘': 'SUPER',
           '⌥': 'ALT', 'OPTION': 'ALT', 'shitty_MOD': 'shitty'}
...
@func_with_args('set_background_opacity', 'goto_layout', 'shitty_shell')
...
def to_layout_names(raw: str) -> List[str]:
    from shitty.layout.interface import all_layouts
...
def shitten_alias(val: str) -> Iterable[Tuple[str, List[str]]]:
...
class BaseDefinition:
    ...
    def resolve_shitten_aliases(self, aliases: Dict[str, List[str]]) -> KeyAction:
        if not self.action.args or not aliases:
            return self.action
        shitten = self.action.args[0]
        ...
        return self.action._replace(args=(shitten, rest.rstrip())) if changed else self.action
...
class MouseMapping(BaseDefinition):
    ...
    def resolve_and_copy(self, shitty_mod: int, aliases: Dict[str, List[str]]) -> 'MouseMapping':
        return MouseMapping(self.button, defines.resolve_key_mods(shitty_mod, self.mods), self.repeat_count, self.grabbed, self.resolve_shitten_aliases(aliases))
...
class KeyDefinition(BaseDefinition):
    ...
    def resolve_and_copy(self, shitty_mod: int, aliases: Dict[str, List[str]]) -> 'KeyDefinition':
        def r(k: SingleKey) -> SingleKey:
            mods = defines.resolve_key_mods(shitty_mod, k.mods)
            return k._replace(mods=mods)
        return KeyDefinition(
            self.is_sequence, self.resolve_shitten_aliases(aliases),
            defines.resolve_key_mods(shitty_mod, self.trigger.mods),
