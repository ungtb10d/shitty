...
from .constants import Version, cache_dir, shitty_exe, version
from .fast_data_types import add_timer, get_boss, monitor_pid
from .notify import notify
from .utils import log_error, open_url

CHANGELOG_URL = 'https://sw.ungtb10d.net/shitty/changelog.html'
RELEASED_VERSION_URL = 'https://sw.ungtb10d.net/shitty/current-version.txt'
...
def notify_new_version(release_version: Version) -> None:
    notify(
            'shitty update available!',
            'shitty version {} released'.format('.'.join(map(str, release_version))),
...
def update_check() -> bool:
    try:
        ...
        log_error('Failed to run shitty for update check, with error: {}'.format(e))
        return False
    monitor_pid(p.pid)
