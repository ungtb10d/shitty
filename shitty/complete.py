...
a simple function that calls shitty's completion code and passes the
results to the shell's completion system. This can be output by
`shitty +complete setup shell_name` and its output goes into
...
is passed to shitty's completion system.

3) An output_serializer function that is responsible for
taking the results from shitty's completion system and converting
...
def load_fish2_completion() -> str:
    with open(os.path.join(shell_integration_dir, 'fish', 'vendor_completions.d', 'shitty.fish')) as f:
        return f.read()
...
_shitty_completions() {
    ...
    src=$(printf "%s\n" "${COMP_WORDS[@]: 0:$limit}" | shitty +complete bash)
    if [[ $? == 0 ]]; then
        eval ${src}
    fi
}
...
def complete_shitty_cli_arg(ans: Completions, opt: Optional[OptionDict], prefix: str, unknown_args: Delegate) -> None:
    ...
    dest = opt['dest']
    if dest == 'override':
        from shitty.config import option_names_for_completion
        k = 'Config directives'
...
def complete_launch_wrapper(ans: Completions, words: Sequence[str], new_word: bool, allow_files: bool = True) -> None:
    from shitty.launch import clone_safe_opts
...
def find_completions(words: Sequence[str], new_word: bool, entry_points: Iterable[str], namespaced_entry_points: Iterable[str]) -> Completions:
    ...
    if exe in ('edit-in-shitty', 'clone-in-shitty'):
        complete_launch_wrapper(ans, words[1:], new_word, allow_files=words[0] != 'clone-in-shitty')
        return ans
    if exe != 'shitty':
        return ans
    words = words[1:]
