...
from .constants import (
    appname, is_macos, is_wayland, read_shitty_resource, shell_path,
...
def load_shaders(name: str) -> Tuple[str, str]:
    ...
    def load(which: str) -> str:
        return read_shitty_resource(f'{name}_{which}.glsl').decode('utf-8').replace('GLSL_VERSION', str(GLSL_VERSION), 1)
...
def set_primary_selection(text: Union[str, bytes]) -> None:
    ...
    from shitty.fast_data_types import set_primary_selection as s
    s(text)
...
def get_primary_selection() -> str:
    if not supports_primary_selection:
        return ''  # There is no primary selection
    from shitty.fast_data_types import get_primary_selection as g
...
def init_startup_notification_x11(window_handle: int, startup_id: Optional[str] = None) -> Optional['StartupCtx']:
    ...
    from shitty.fast_data_types import init_x11_startup_notification
...
def end_startup_notification_x11(ctx: 'StartupCtx') -> None:
    from shitty.fast_data_types import end_x11_startup_notification
    ...
    try:
        try:
            ...
            log_error(
                ...
                'There will be no startup feedback and when using --single-instance, shitty windows may start on an incorrect desktop/workspace.')
    except Exception:
        import traceback
        traceback.print_exc()
...
def edit_config_file() -> None:
    from shitty.config import prepare_config_file_for_editing
