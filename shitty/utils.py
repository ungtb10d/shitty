...
from .constants import (
    ...
    read_shitty_resource, runtime_dir, shell_path, ssh_control_master_template
)
...
@run_once
def shitty_ansi_sanitizer_pat() -> 're.Pattern[str]':
    # removes ANSI sequences generated by shitty's ANSI output routines. Not
...
def load_shaders(name: str, vertex_name: str = '', fragment_name: str = '') -> Tuple[str, str]:
    ...
    def load(which: str, lname: str = '') -> str:
        lname = lname or name
        return read_shitty_resource(f'{lname}_{which}.glsl').decode('utf-8').replace('GLSL_VERSION', str(GLSL_VERSION), 1)
...
def get_all_processes() -> Iterable[int]:
    if is_macos:
        from shitty.fast_data_types import get_all_processes as f
        yield from f()
    else:
...
def is_shitty_gui_cmdline(*cmd: str) -> bool:
    if not cmd:
        return False
    if os.path.basename(cmd[0]) != 'shitty':
        return False
    if len(cmd) == 1:
        return True
    s = cmd[1][:1]
...
def reload_conf_in_all_kitties() -> None:
    ...
    for pid in get_all_processes():
        try:
            ...
            continue
        if cmd and is_shitty_gui_cmdline(*cmd):
...
def cleanup_ssh_control_masters() -> None:
    ...
    try:
        files = frozenset(glob.glob(os.path.join(runtime_dir(), ssh_control_master_template.format(
            shitty_pid=os.getpid(), ssh_placeholder='*'))))
    except OSError:
        return
    workers = tuple(subprocess.Popen([
        'ssh', '-o', f'ControlPath={x}', '-O', 'exit', 'shitty-unused-host-name'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
...
def path_from_osc7_url(url: str) -> str:
    if url.startswith('shitty-shell-cwd://'):
