...
- (void)open_shitty_website_url:(id)sender {
    (void)sender;
    [[NSWorkspace sharedWorkspace] openURL:[NSURL URLWithString:@"https://sw.ungtb10d.net/shitty/"]];
}
...
#define MENU_ITEM(menu, title, name) { \
    ...
    [[helpMenu addItemWithTitle:[NSString stringWithFormat:@"Visit %@ website", app_name]
                         action:@selector(open_shitty_website_url:)
                  keyEquivalent:@"?"]
                      setTarget:global_menu_target];
