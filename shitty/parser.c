        ...
        case 'm':
            if (start_modifier == '>' && !end_modifier) {
                REPORT_ERROR(
                    ...
                    " This is superseded by the shitty keyboard protocol: https://sw.ungtb10d.net/shitty/keyboard-protocol/"
...
dispatch_dcs(Screen *screen, PyObject DUMP_UNUSED *dump_callback) {
    ...
    switch(screen->parser_buf[0]) {
        ...
        case PENDING_MODE_CHAR:
            ...
            break;
        case '@':
            if (startswith(screen->parser_buf + 1, screen->parser_buf_pos - 2, "shitty-", sizeof("shitty-") - 1)) {
...
#define IF_SIMPLE_PREFIX(prefix, func) \
        if (startswith(screen->parser_buf + 7, screen->parser_buf_pos - 1, prefix, sizeof(prefix) - 1)) { \
            const size_t pp_size = sizeof("shitty") + sizeof(prefix); \
