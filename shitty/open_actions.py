...
protocol file
ext sh,command,tool
action launch --hold --type=os-window shitty +shebang $FILE_PATH {SHELL}

# Open shell specific script files
protocol file
ext fish,bash,zsh
action launch --hold --type=os-window shitty +shebang $FILE_PATH __ext__
...
protocol file
mime image/*
action launch --type=os-window shitty +shitten icat --hold $FILE_PATH
...
def actions_for_launch(url: str) -> Iterator[KeyAction]:
    # Custom launch actions using shitty URL scheme needs to be prefixed with `shitty:///launch/`
    if url.startswith('shitty://') and not url.startswith('shitty:///launch/'):
        return
    found = False
