...
Where to launch the child process, in a new shitty window in the current tab,
...
shitty remote control feature with :code:`shitty @launch --copy-env`.
...
type=bool-set
Programs running in this window can control shitty (if remote control is
...
control shitty. It can, however, be useful to block programs running on other
...
Defaults to whatever is used by the parent shitty process, which in turn
defaults to :code:`shitty`.
...
file with the same syntax as shitty.conf to read the colors from, or specify them
