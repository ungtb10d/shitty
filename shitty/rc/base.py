...
from shitty.constants import appname, running_in_shitty

if TYPE_CHECKING:
    from shitty.boss import Boss as B
    from shitty.tabs import Tab
    from shitty.window import Window as W
    Window = W
    Boss = B
    Tab
else:
...
is available as the shitty_WINDOW_ID environment variable. When using the :italic:`env` field
...
def command_for_name(cmd_name: str) -> RemoteCommand:
    ...
    try:
        m = import_module(f'shitty.rc.{cmd_name}')
    except ImportError:
        raise KeyError(f'{cmd_name} is not a known shitty remote control command')
...
def all_command_names() -> FrozenSet[str]:
    ...
    return frozenset({x.rpartition('.')[0] for x in filter(ok, contents('shitty.rc'))})
