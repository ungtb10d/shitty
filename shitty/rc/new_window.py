...
from shitty.fast_data_types import focus_os_window
from shitty.tabs import SpecialWindow
...
if TYPE_CHECKING:
    from shitty.cli_stub import NewWindowRCOptions as CLIOptions
...
class NewWindow(RemoteCommand):
    ...
    window_type: One of :code:`shitty` or :code:`os`
    keep_focus: Boolean indicating whether the current window should retain focus or not
    '''

    short_desc = 'Open new window'
    desc = (
        'Open a new window in the specified tab. If you use the :option:`shitty @ new-window --match` option'
        ...
        ':italic:`shitty @ new-window --title Email mutt`'
    )
...
the working directory for the shitty process you are talking to is.
...
default=shitty
choices=shitty,os
What kind of window to open. A shitty window or a top-level OS window.
