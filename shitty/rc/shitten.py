...
if TYPE_CHECKING:
    from shitty.cli_stub import KittenRCOptions as CLIOptions
...
class shitten(RemoteCommand):
    ...
    desc = (
        ...
        ' The :italic:`kitten_name` can be either the name of a builtin shitten'
    ...
    def response_from_kitty(self, boss: Boss, window: Optional[Window], payload_get: PayloadGetType) -> ResponseType:
        ...
        for window in self.windows_for_match_payload(boss, window, payload_get):
            if window:
                retval = boss.run_kitten_with_metadata(payload_get('shitten'), args=tuple(payload_get('args') or ()), window=window)
                break
