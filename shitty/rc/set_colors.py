...
from shitty.config import parse_config
from shitty.fast_data_types import patch_color_profiles
from shitty.rgb import Color
...
if TYPE_CHECKING:
    from shitty.cli_stub import SetColorsRCOptions as CLIOptions
...
class SetColors(RemoteCommand):
    ...
    desc = (
        ...
        ' (in the same format as shitty.conf) to read the colors from or you can specify individual colors,'
        ' for example: shitty @ set-colors foreground=red background=white'
    )
    options_spec = '''\
--all -a
type=bool-set
...
--configured -c
type=bool-set
Also change the configured colors (i.e. the colors shitty will use for new
windows or after a reset).


--reset
type=bool-set
Restore all colors to the values they had at shitty startup. Note that if you specify
