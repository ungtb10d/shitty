...
from shitty.cli import emph
from shitty.config import parse_config
from shitty.fast_data_types import patch_color_profiles, Color
...
class SetColors(RemoteCommand):
    ...
    desc = (
        ...
        ' (in the same format as :file:`shitty.conf`) to read the colors from or you can specify individual colors,'
        ' for example::\n\n'
        '    shitty @ set-colors foreground=red background=white'
    )
    options_spec = '''\
--all -a
type=bool-set
...
this option, any color arguments are ignored and :option:`shitty @ set-colors --configured` and :option:`shitty @ set-colors --all` are implied.
