...
class Boss:
    ...
    def _handle_remote_command(self, cmd: str, window: Optional[Window] = None, peer_id: int = 0) -> RCResponse:
        ...
        if q is False and pcmd.get('password'):
            response['error'] = 'The user rejected this password or it is disallowed by remote_control_password in shitty.conf'
    ...
    def ask_if_remote_cmd_is_allowed(self, pcmd: Dict[str, Any], window: Optional[Window] = None, peer_id: int = 0) -> bool:
        ...
        overlay_window = self.choose(
            _('A program wishes to control shitty.\n'
              'Action: {1}\n' 'Password: {0}\n\n' '{2}'
              ).format(
                  hidden_text, styled(pcmd['cmd'], fg='magenta'),
                  '\x1b[m' + styled(_(
                      'Note that allowing the password will allow all future actions using the same password, in this shitty instance.'
    ...
    def peer_message_received(self, msg_bytes: bytes, peer_id: int) -> Union[bytes, bool, None]:
        ...
        if msg_bytes.startswith(cmd_prefix) and msg_bytes.endswith(terminator):
            ...
            if isinstance(response, AsyncResponse):
                return True
            from shitty.remote_control import encode_response_for_peer
    ...
    @ac('win', '''
        ...
        See :ref:`conf-shitty-shortcuts.fonts` for details.
        ''')
    ...
    def quit(self, *args: Any) -> None:
        ...
        self.confirm(
            _('Are you sure you want to quit shitty, it has {} windows running?').format(num),
    ...
    def patch_colors(self, spec: Dict[str, Optional[int]], configured: bool = False) -> None:
        from shitty.rc.set_colors import nullable_colors
        opts = get_options()
        if configured:
    ...
    @ac('misc', '''
        ...
        config options. For example::
            ...
            map f5 load_config_file /path/to/some/shitty.conf
        ''')
    ...
    def set_colors(self, *args: str) -> None:
        from shitty.rc.base import PayloadGetter, command_for_name, parse_subcommand_cli
    ...
    def show_kitty_env_vars(self) -> None:
        ...
        if w:
            ...
            self.display_scrollback(w, output, title=_('Current shitty env vars'), report_cursor=False)
    ...
    @ac('debug', 'Show the effective configuration shitty is running with')
    def debug_config(self) -> None:
        ...
        if w is not None:
            ...
            self.display_scrollback(w, output, title=_('Current shitty options'), report_cursor=False)
