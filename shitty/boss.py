...
from .constants import (
    appname, config_dir, is_macos, is_wayland, shitty_exe,
...
class Boss:
    ...
    def _handle_remote_command(self, cmd: str, window: Optional[Window] = None, from_peer: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if get_options().allow_remote_control == 'y' or from_peer or getattr(window, 'allow_remote_control', False):
            ...
            try:
                ...
                pass
            if not no_response:
                response = {'ok': False, 'error': 'Remote control is disabled. Add allow_remote_control to your shitty.conf'}
        return response
    ...
    def remote_control(self, *args: str) -> None:
        ...
        try:
            ...
            if isinstance(cast(types.GeneratorType, payload), types.GeneratorType):
                ...
                for x in payloads:
                    c.response_from_shitty(self, self.active_window, PayloadGetter(c, x if isinstance(x, dict) else {}))
            else:
    ...
    def confirm_tab_close(self, tab: Tab) -> None:
        ...
        if not needs_confirmation:
            ...
            return
        self._run_shitten('ask', ['--type=yesno', '--message', _(
    ...
    def set_background_opacity(self, opacity: str) -> None:
        ...
        if not get_options().dynamic_background_opacity:
            self.show_error(
                    ...
                    _('You must set the dynamic_background_opacity option in shitty.conf to be able to change background opacity'))
            return
        os_window_id = window.os_window_id
        if opacity[0] in '+-':
    ...
    def start_resizing_window(self) -> None:
        ...
        if w is None:
            return
        overlay_window = self._run_shitten('resize_window', args=[
    ...
    def quit(self, *args: Any) -> None:
        ...
        self._run_shitten('ask', ['--type=yesno', '--message', _(
            'Are you sure you want to quit shitty, it has {} windows running?').format(num)],
    ...
    def edit_config_file(self, *a: Any) -> None:
        ...
        # small delay.
        cmd = [shitty_exe(), '+runpy', 'import os, sys, time; time.sleep(0.05); os.execvp(sys.argv[1], sys.argv[1:])'] + get_editor(get_options()) + [confpath]
    ...
    def _run_shitten(
        ...
        if w is not None and tab is not None:
            ...
            wid = w.id
            overlay_window.action_on_close = partial(self.on_shitten_finish, wid, custom_callback or end_shitten)
    ...
    def shitten(self, shitten: str, *args: str) -> None:
        import shlex
        cmdline = args[0] if args else ''
        kargs = shlex.split(cmdline) if cmdline else []
        self._run_shitten(shitten, kargs)

    def run_shitten(self, shitten: str, *args: str) -> None:
        self._run_shitten(shitten, args)

    def on_shitten_finish(self, target_window_id: str, end_shitten: Callable, source_window: Window) -> None:
        output = self.get_output(source_window, num_lines=None)
        from shittens.runner import deserialize
        data = deserialize(output)
        if data is not None:
            end_shitten(data, target_window_id, self)

    def input_unicode_character(self) -> None:
        self._run_shitten('unicode_input')
    ...
    def show_error(self, title: str, msg: str) -> None:
        self._run_shitten('show_error', args=['--title', title], input_data=msg)
    ...
    def create_marker(self) -> None:
        ...
        if w:
            ...
            self._run_shitten('ask', [
                '--name=create-marker', '--message',
                _('Create marker, for example:\ntext 1 ERROR\nSee https://sw.ungtb10d.net/shitty/marks.html\n')
                ],
                custom_callback=done, action_on_removal=done2)

    def shitty_shell(self, window_type: str) -> None:
        kw: Dict[str, Any] = {}
        cmd = [shitty_exe(), '@']
        aw = self.active_window
        if aw is not None:
            kw['env'] = {'shitty_SHELL_ACTIVE_WINDOW_ID': str(aw.id)}
        if window_type == 'tab':
    ...
    def launch(self, *args: str) -> None:
        from shitty.launch import launch, parse_launch_args
    ...
    def show_bad_config_lines(self, bad_lines: Iterable[BadLine]) -> None:
        ...
        self.show_error(_('Errors in shitty.conf'), msg)

    def set_colors(self, *args: str) -> None:
        from shitty.rc.base import (
            PayloadGetter, command_for_name, parse_subcommand_cli
        )
        from shitty.remote_control import parse_rc_args
    ...
    # Can be called with shitty -o "map f1 send_test_notification"
    ...
    def show_shitty_env_vars(self) -> None:
        ...
        if w:
            ...
            self.display_scrollback(w, output, title=_('Current shitty env vars'))
    ...
    def debug_config(self) -> None:
        ...
        if w is not None:
            ...
            self.display_scrollback(w, output, title=_('Current shitty options'))
