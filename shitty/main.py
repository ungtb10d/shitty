...
from .constants import (
    ...
    is_wayland, shitty_exe, logo_png_file, running_in_shitty
)
...
class AppRunner:
    ...
    def __call__(self, opts: Options, args: CLIOptions, bad_lines: Sequence[BadLine] = ()) -> None:
        ...
        try:
            ...
            if is_macos:
                from shitty.fast_data_types import cocoa_set_notification_activated_callback
...
def setup_environment(opts: Options, cli_opts: CLIOptions) -> None:
    ...
    if cli_opts.listen_on and opts.allow_remote_control != 'n':
        ...
        os.environ['shitty_LISTEN_ON'] = cli_opts.listen_on
...
def _main() -> None:
    ...
    if rpath:
        modify_path = is_macos or getattr(sys, 'frozen', False) or sys._xoptions.get('shitty_from_source') == '1'
        if modify_path or not shutil.which('shitty'):
