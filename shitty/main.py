    ...
    if candidates:
        ...
        for candidate in reversed(candidates):
            if candidate.mods in alt_mods:
                ...
                # https://github.com/ungtb10d/shitty/issues/3515
                continue
...
def set_macos_app_custom_icon() -> None:
    for name in ('shitty.app.icns', 'shitty.app.png'):
        ...
        if custom_icon_mtime is not None:
            ...
            # the application icon here. This will revert when shitty quits, but
...
def ensure_macos_locale() -> None:
    ...
    # https://github.com/ungtb10d/shitty/issues/90
...
def setup_profiling() -> Generator[None, None, None]:
    try:
        ...
        do_profile = False
    if do_profile:
        start_profiler('/tmp/shitty-profile.log')
    yield
    if do_profile:
        import subprocess
        stop_profiler()
        ...
        with open(cg, 'wb') as f:
            subprocess.call(['pprof', '--callgrind', exe, '/tmp/shitty-profile.log'], stdout=f)
        try:
            subprocess.Popen(['kcachegrind', cg], preexec_fn=clear_handled_signals)
        except FileNotFoundError:
            subprocess.call(['pprof', '--text', exe, '/tmp/shitty-profile.log'])
...
def prepend_if_not_present(path: str, paths_serialized: str) -> str:
    # prepend a path only if path/shitty is not already present, even as a symlink
    pq = os.path.join(path, 'shitty')
    for candidate in paths_serialized.split(os.pathsep):
        q = os.path.join(candidate, 'shitty')
...
def setup_manpath(env: Dict[str, str]) -> None:
    # Ensure shitty manpages are available in frozen builds
...
def _main() -> None:
    ...
    if getattr(sys, 'cmdline_args_for_open', False):
        ...
        appname: Optional[str] = 'shitty +open'
        msg: Optional[str] = (
            'Run shitty and open the specified files or URLs in it, using launch-actions.conf. For details'
            ' see https://sw.ungtb10d.net/shitty/open_actions/#scripting-the-opening-of-files-with-shitty-on-macos'
            '\n\nAll the normal shitty options can be used.')
    else:
    ...
    if cli_opts.replay_commands:
        from shitty.client import main as client_main
    ...
    if cli_opts.watcher:
        ...
        log_error('The --watcher command line option has been deprecated in favor of using the watcher option in shitty.conf')
    try:
