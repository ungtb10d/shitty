...
#define SIDE_EFFECTS(name) case name: if (do_side_effects) set_mode_from_const(self, name, src->m##name); else dest->m##name = src->m##name; break;
    ...
    switch(mode) {
        SIMPLE_MODE(LNM)  // shitty extension
        SIMPLE_MODE(IRM)  // shitty extension
        SIMPLE_MODE(DECARM)
        SIMPLE_MODE(BRACKETED_PASTE)
        SIMPLE_MODE(FOCUS_TRACKING)
        SIMPLE_MODE(DECCKM)
...
screen_save_modes(Screen *self) {
    // shitty extension to XTSAVE that saves a bunch of no side-effect modes
...
screen_restore_modes(Screen *self) {
    // shitty extension to XTRESTORE that saves a bunch of no side-effect modes
...
screen_xtversion(Screen *self, unsigned int mode) {
    if (mode == 0) {
        write_escape_code_to_child(self, DCS, ">|shitty(" XT_VERSION ")");
    }
}
