...
create_os_window(PyObject UNUSED *self, PyObject *args, PyObject *kw) {
    ...
    if (!apple_preserve_common_context) {
        apple_preserve_common_context = glfwCreateWindow(640, 480, "shitty", NULL, common_context);
    }
    ...
    if (!global_state.is_wayland) {
        ...
        if (temp_window == NULL) { fatal("Failed to create GLFW temp window! This usually happens because of old/broken OpenGL drivers. shitty requires working OpenGL 3.3 drivers."); }
    }
    float xscale, yscale;
    double xdpi, ydpi;
...
#else
    if (OPT(bell_path)) play_canberra_sound(OPT(bell_path), "shitty bell", true, "event");
    else play_canberra_sound("bell", "shitty bell", false, "event");
#endif
}
...
request_frame_render(OSWindow *w) {
    ...
    // render frame requests, see https://github.com/ungtb10d/shitty/issues/2329
