...
bool_capabilities = {
    ...
    # described at:
    # https://github.com/ungtb10d/shitty/blob/master/protocol-extensions.asciidoc
    'Su',
...
def get_capabilities(query_string: str, opts: 'Options') -> Generator[str, None, None]:
    ...
    for encoded_query_name in query_string.split(';'):
        ...
        elif name.startswith('shitty-query-'):
            from shittens.query_terminal.main import get_result
            name = name[len('shitty-query-'):]
            rval = get_result(name)
            if rval is None:
                from .utils import log_error
                log_error('Unknown shitty terminfo query:', name)
