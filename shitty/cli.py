...
If the environment variable :env:`shitty_CONFIG_DIRECTORY` is specified, that
...
def doc(x: str) -> str:
    return f'https://sw.ungtb10d.net/shitty/{x}.html'
...
def version(add_rev: bool = False) -> str:
    ...
    if add_rev and hasattr(fast_data_types, 'shitty_VCS_REV'):
        rev = ' ({})'.format(fast_data_types.shitty_VCS_REV[:10])
...
For comprehensive documentation for shitty, please see: https://sw.ungtb10d.net/shitty/''').format(appname=appname)
...
class PrintHelpForSeq:
    ...
    def __call__(self, seq: OptionSpecSeq, usage: Optional[str], message: Optional[str], appname: str) -> None:
        from shitty.utils import screen_size_function
...
resolved relative to the shitty config directory.'''
...
--title -T
Set the window title. This will override any title set by the program running inside shitty. So
...
Syntax: :italic:`name=value`. For example: :option:`shitty -o` font_size=20
...
:option:`shitty --instance-group` option


--instance-group
Used in combination with the :option:`shitty --single-instance` option. All :italic:`{appname}` invocations
with the same :option:`shitty --instance-group` will result in new windows being created
...
--listen-on
Tell shitty to listen on the specified address for control
messages. For example, :option:`{appname} --listen-on`=unix:/tmp/myshitty or
...
:option:`{appname} --listen-on`=unix:@myshitty. Environment variables
in the setting are expanded and relative paths are resolved with
respect to the temporary directory. To control shitty, you can send
it commands with :italic:`shitty @` using the :option:`shitty @ --to` option to
specify this address. This option will be ignored, unless you set
:opt:`allow_remote_control` to yes in :file:`shitty.conf`. Note that if you run
:italic:`shitty @` within a shitty window, there is no need to specify the :italic:`--to`
option as it is read automatically from the environment. For UNIX sockets, this
can also be specified in :file:`shitty.conf`.
...
Control how the initial shitty window is created.
...
can open a new shitty window to replay the commands with::

    shitty sh -c "shitty --replay-commands /path/to/dump/file; read"
...
dest=debug_keyboard
type=bool-set
This option will cause shitty to print out key and mouse events as they are received
...
SYSTEM_CONF = '/etc/xdg/shitty/shitty.conf'
