...
def install_deps():
    print('Installing shitty dependencies...')
    sys.stdout.flush()
    if is_macos:
...
def test_shitty():
    run('./shitty/launcher/shitty +launch test.py')
...
def setup_bundle_env():
    global SW
    os.environ['SW'] = SW = '/Users/Shared/shitty-build/sw/sw' if is_macos else os.path.join(os.environ['GITHUB_WORKSPACE'], 'sw')
...
def install_bundle():
    ...
    os.makedirs(SW)
    os.chdir(SW)
    with urlopen('https://download.calibre-ebook.com/ci/shitty/{}-64.tar.xz'.format(
