...
def main(args: List[str]) -> Response:
    # For some reason importing readline in a key handler in the main shitty process
    # causes a crash of the python interpreter, probably because of some global
    # lock
    global readline
    msg = 'Ask the user for input'
    try:
        cli_opts, items = parse_args(args[1:], option_text, '', msg, 'shitty ask', result_class=AskCLIOptions)
    ...
    readline = rl
    from shitty.shell import init_readline
    init_readline()
    response = None
