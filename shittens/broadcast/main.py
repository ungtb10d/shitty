...
from shitty.cli_stub import BroadcastCLIOptions
from shitty.key_encoding import encode_key_event
from shitty.rc.base import MATCH_TAB_OPTION, MATCH_WINDOW_OPTION
from shitty.remote_control import create_basic_command, encode_send
from shitty.short_uuid import uuid4
from shitty.typing import KeyEventType, ScreenSize
...
class Broadcast(Handler):
    ...
    def on_key(self, key_event: KeyEventType) -> None:
        ...
        self.write_broadcast_data('shitty-key:' + ek)
...
help_text = 'Broadcast typed text to shitty windows. By default text is sent to all windows, unless one of the matching options is specified'
usage = '[initial text to send ...]'


def parse_broadcast_args(args: List[str]) -> Tuple[BroadcastCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'shitty +shitten broadcast', result_class=BroadcastCLIOptions)
