...
from shitty.cli import CONFIG_HELP, parse_args
from shitty.cli_stub import DiffCLIOptions
from shitty.conf.utils import KeyAction
from shitty.constants import appname
from shitty.fast_data_types import wcswidth
from shitty.key_encoding import EventType, KeyEvent
...
def main(args: List[str]) -> None:
    ...
    cli_opts, items = parse_args(args[1:], OPTIONS, usage, help_text, 'shitty +shitten diff', result_class=DiffCLIOptions)
    if len(items) != 2:
    ...
    for message in showwarning.warnings:
        from shitty.utils import safe_print
