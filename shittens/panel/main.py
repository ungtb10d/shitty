...
from shitty.cli_stub import PanelCLIOptions
from shitty.constants import is_macos, appname
from shitty.os_window_size import WindowSizeData
...
--config -c
type=list
Path to config file to use for shitty when drawing the panel.


--override -o
type=list
Override individual shitty configuration options, can be specified multiple times.
Syntax: :italic:`name=value`. For example: :option:`shitty +shitten panel -o` font_size=20
...
def parse_panel_args(args: List[str]) -> Tuple[PanelCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'shitty +shitten panel', result_class=PanelCLIOptions)
...
def initial_window_size_func(opts: WindowSizeData, cached_values: Dict) -> Callable[[int, int, float, float, float, float], Tuple[int, int]]:
    from shitty.fast_data_types import glfw_primary_monitor_size
...
def main(sys_args: List[str]) -> None:
    ...
    if is_macos or not os.environ.get('DISPLAY'):
        raise SystemExit('Currently the panel shitten is supported only on X11 desktops')
    if not shutil.which('xprop'):
        raise SystemExit('The xprop program is required for the panel shitten')
    ...
    from shitty.main import run_app, main as real_main
