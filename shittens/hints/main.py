...
from shitty.clipboard import set_clipboard_string, set_primary_selection
...
:code:`launch`
    run :doc:`/launch` to open the program in a new shitty tab, window, overlay, etc.
    For example::
...
The actual action is whatever arguments are provided to the shitten, for
example:
:code:`shitty +shitten hints --type=linenum --linenum-action=tab vim +{line} {path}`
will open the matched path at the matched line number in vim in
a new shitty tab. Note that in order to use :option:`--program` to copy or paste
...
prefixes defined by the :opt:`url_prefixes` option in :file:`shitty.conf`.
...
Defaults to the :opt:`select_by_word_characters` option from :file:`shitty.conf`.
...
Name of a python file in the shitty config directory which will be imported to
...
def handle_result(args: List[str], data: Dict[str, Any], target_window_id: int, boss: BossType) -> None:
    ...
    for program in programs:
        if program == '-':
            ...
            from shitty.conf.utils import to_cmdline
            cwd = data['cwd']
