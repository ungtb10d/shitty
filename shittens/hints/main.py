...
from shitty.cli_stub import HintsCLIOptions
from shitty.fast_data_types import set_clipboard_string
from shitty.key_encoding import KeyEvent
from shitty.typing import BossType, shittyCommonOpts
from shitty.utils import ScreenSize, screen_size_function, set_primary_selection
...
def load_custom_processor(customize_processing: str) -> Any:
    ...
    from shitty.constants import resolve_custom_file
...
The regular expression to use when :option:`shitty +shitten hints --type`=regex.
...
If you specify named groups and a :option:`shitty +shitten hints --program` then
...
window, :code:`window` a new shitty window, :code:`tab` a new tab,
...
arguments are provided to the shitten, for example: :code:`shitty + shitten hints
--type=linenum --linenum-action=tab vim +{line} {path}` will open the matched
path at the matched line number in vim in a new shitty tab.


--url-prefixes
default=default
Comma separated list of recognized URL prefixes. Defaults, to
the list of prefixes defined in shitty.conf.
...
Defaults to the select_by_word_characters setting from shitty.conf.
...
Name of a python file in the shitty config directory which will be imported to provide
custom implementations for pattern finding and performing actions
on selected matches. See https://sw.ungtb10d.net/shitty/shittens/hints.html
...
def parse_hints_args(args: List[str]) -> Tuple[HintsCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'shitty +shitten hints', result_class=HintsCLIOptions)
