...
def main() -> None:
    ...
    def parse_link_options(raw: str) -> None:
        ...
        if not raw:
            raise SystemExit('Must specify an argument for --shitten option')
        p, _, s = raw.partition('=')
        if p != 'hyperlink':
            raise SystemExit(f'Unknown argument for --shitten: {raw}')
    ...
    while i < len(sys.argv):
        if sys.argv[i] == '--shitten':
            next_item = '' if i + 1 >= len(sys.argv) else sys.argv[i + 1]
            parse_link_options(next_item)
            del sys.argv[i:i+2]
        elif sys.argv[i].startswith('--shitten='):
            parse_link_options(sys.argv[i][len('--shitten='):])
            del sys.argv[i]
        else:
            i += 1
