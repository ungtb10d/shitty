...
from shitty.cli_stub import TransferCLIOptions
...
The password to use to skip the transfer confirmation popup in shitty. Must match
the password set for the :opt:`file_transfer_confirmation_bypass` option in
:file:`shitty.conf`. Note that leading and trailing whitespace is removed from
...
def parse_transfer_args(args: List[str]) -> Tuple[TransferCLIOptions, List[str]]:
    return parse_args(
        ...
        'shitty +shitten transfer', result_class=TransferCLIOptions
    )
...
def main(args: List[str]) -> None:
    ...
    if not items:
        raise SystemExit('Usage: shitty +shitten transfer file_or_directory ...')
