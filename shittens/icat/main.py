...
from shitty.cli_stub import IcatCLIOptions
from shitty.constants import appname
from shitty.guess_mime_type import guess_type
from shitty.rgb import to_color
from shitty.types import run_once
from shitty.typing import GRT_f, GRT_t
...
convenience method to query the window size if using :code:`shitty +shitten icat`
...
def main(args: List[str] = sys.argv) -> None:
    global can_transfer_with_files
    cli_opts, items_ = parse_args(args[1:], options_spec, usage, help_text, f'{appname} +shitten icat', result_class=IcatCLIOptions)
    ...
    if cli_opts.transfer_mode == 'detect':
        if not detect_support(wait_for=cli_opts.detection_timeout, silent=cli_opts.silent):
            raise SystemExit('This terminal emulator does not support the graphics protocol, use a terminal emulator such as shitty that does support it')
    else:
