...
import typing
from shittens.ssh.options.utils import copy, env, hostname
from shitty.conf.utils import merge_dicts, to_bool
...
class Parser:
    ...
    def remote_shitty(self, val: str, ans: typing.Dict[str, typing.Any]) -> None:
        val = val.lower()
        if val not in self.choices_for_remote_shitty:
            raise ValueError(f"The value {val} is not a valid choice for remote_shitty")
        ans["remote_shitty"] = val

    choices_for_remote_shitty = frozenset(('if-needed', 'no', 'yes'))
