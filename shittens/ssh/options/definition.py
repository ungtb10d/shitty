...
opt('remote_dir', '.local/share/shitty-ssh-shitten', long_text='''
The location on the remote host where the files needed for this shitten are
...
use the setting from :file:`shitty.conf`. This setting is useful for overriding
...
copied which will download and run shitty when shitty is first executed on the
remote host. A value of :code:`if-needed` means shitty is installed only if not
already present in the system-wide PATH. A value of :code:`yes` means that shitty
is installed even if already present, and the installed shitty takes precedence.
Finally, :code:`no` means no shitty is installed on the remote host. The
installed shitty can be updated by running: :code:`shitty +update-shitty` on the
remote host.
''')
egr()  # }}}
...
Within a single shitty instance, all connections to a particular server can be
...
ControlMasters and these are automatically cleaned up by shitty when it quits.
...
etc. The default is to use shitty's native :program:`askpass`, unless the
...
to always use shitty's native, built-in askpass implementation. Note that not
using the shitty askpass implementation means that SSH might need to use the
terminal before the connection is established, so the shitten cannot use the
