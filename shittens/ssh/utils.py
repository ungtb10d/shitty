...
def is_shitten_cmdline(q: Sequence[str]) -> bool:
    if len(q) < 4:
        return False
    if os.path.basename(q[0]).lower() != 'shitty':
        return False
    if q[1:3] == ['+shitten', 'ssh'] or q[1:4] == ['+', 'shitten', 'ssh']:
        return True
    return q[1:3] == ['+runpy', 'from shittens.runner import main; main()'] and len(q) >= 6 and q[5] == 'ssh'


def patch_cmdline(key: str, val: str, argv: List[str]) -> None:
    for i, arg in enumerate(tuple(argv)):
        if arg.startswith(f'--shitten={key}='):
            argv[i] = f'--shitten={key}={val}'
            return
        elif i > 0 and argv[i-1] == '--shitten' and (arg.startswith(f'{key}=') or arg.startswith(f'{key} ')):
            argv[i] = val
            return
    idx = argv.index('ssh')
    argv.insert(idx + 1, f'--shitten={key}={val}')
