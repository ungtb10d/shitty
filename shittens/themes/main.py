...
from shitty.typing import KeyEventType
from shitty.utils import ScreenSize
...
class ThemesHandler(Handler):
    ...
    def init_terminal_state(self) -> None:
        ...
        self.cmd.set_window_title('Choose a theme for shitty')
...
help_text = (
    'Change the shitty theme. If no theme name is supplied, run interactively, otherwise'
...
By default, this shitten will signal only the parent shitty instance it is
...
instead of changing shitty.conf.


--config-file-name
default=shitty.conf
The name or path to the config file to edit. Relative paths are interpreted
with respect to the shitty config directory. By default the shitty config file,
shitty.conf is edited.
'''.format


def parse_themes_args(args: List[str]) -> Tuple[ThemesCLIOptions, List[str]]:
    return parse_args(args, OPTIONS, usage, help_text, 'shitty +shitten themes', result_class=ThemesCLIOptions)
...
def main(args: List[str]) -> None:
    ...
    loop = Loop()
    with cached_values_for('themes-shitten') as cached_values:
