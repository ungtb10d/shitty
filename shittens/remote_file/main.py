...
from shitty.cli_stub import RemoteFileCLIOptions
from shitty.constants import cache_dir
from shitty.typing import BossType
    ...
    def check_hostname_matches(self) -> bool:
        ...
        if cp.returncode == 0:
            ...
            if not hostname_matches(self.cli_opts.hostname or '', q):
                ...
                print('This indicates that shitty has not connected to the correct remote machine.')
                print('This can happen, for example, when using nested SSH sessions.')
                print(f'The hostname shitty used to connect was: {styled(self.conn_data.hostname, fg="yellow")}', end='')
...
def main(args: List[str]) -> Result:
    ...
    try:
        cli_opts, items = parse_args(args[1:], option_text, '', msg, 'shitty remote_file', result_class=RemoteFileCLIOptions)
