...
from shitty.cli_stub import ClipboardCLIOptions
from shitty.fast_data_types import parse_input_from_terminal
...
shitty will prompt for permission to access the clipboard. Can be controlled
...
the shitten in a dedicated, ephemeral window.
'''.format
help_text = '''\
...
def main(args: List[str]) -> NoReturn:
    cli_opts, items = parse_args(args[1:], OPTIONS, usage, help_text, 'shitty +shitten clipboard', result_class=ClipboardCLIOptions)
    if items:
