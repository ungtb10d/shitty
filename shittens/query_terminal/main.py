...
from shitty.cli_stub import QueryTerminalCLIOptions
from shitty.constants import appname, str_version
from shitty.options.types import Options
from shitty.terminfo import names
from shitty.utils import TTYIO
...
class Query:
    ...
    def query_name(self) -> str:
        return self.override_query_name or f'shitty-query-{self.name}'
...
class AllowHyperlinks(Query):
    ...
    help_text: str = 'The config option :opt:`allow_hyperlinks` in :file:`shitty.conf` for allowing hyperlinks can be :code:`yes`, :code:`no` or :code:`ask`'
...
class FontFamily(Query):
    ...
    @staticmethod
    def get_result(opts: Options) -> str:
        from shitty.fast_data_types import current_fonts
        cf = current_fonts()
...
class ClipboardControl(Query):
    ...
    help_text: str = 'The config option :opt:`clipboard_control` in :file:`shitty.conf` for allowing reads/writes to/from the clipboard'
...
help_text = '''\
Query the terminal this shitten is run in for various capabilities. This sends
...
If a particular :italic:`query` is unsupported by the running shitty version, the
:italic:`data` will be blank.

Note that when calling this from another program, be very careful not to perform
any I/O on the terminal device until this shitten exits.
...
def main(args: List[str] = sys.argv) -> None:
    cli_opts, items_ = parse_args(
        ...
        usage,
        help_text,
        f'{appname} +shitten query_terminal',
