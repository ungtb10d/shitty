...
class Freeze(object):
    ...
    @flush
    def run_tests(self):
        iv['run_tests'](os.path.join(self.contents_dir, 'MacOS', 'shitty'))
    ...
    def freeze_python(self):
        ...
        os.rename(os.path.join(shitty_dir, '__main__.py'), os.path.join(self.python_stdlib, 'shitty_main.py'))
        shutil.rmtree(os.path.join(shitty_dir, '__pycache__'))
        pdir = os.path.join(dirname(self.python_stdlib), 'shitty-extensions')
        os.mkdir(pdir)
        ...
        self.fix_dependencies_in_lib(join(self.contents_dir, 'MacOS', 'shitty'))
        for f in walk(pdir):
...
def main():
    ...
    Freeze(
        os.path.join(ext_dir, shitty_constants['appname'] + '.app'),
