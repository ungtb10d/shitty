...
The ssh shitten allows you to login easily to remote hosts, and automatically
...
shitty terminfo database there.

The ssh shitten is a thin wrapper around the traditional `ssh <https://man.openbsd.org/ssh>`__
...
.. code-block:: sh
    ...
    shitty +shitten ssh some-hostname-to-connect-to
...
If you define a mapping in :file:`shitty.conf` such as::

    map f1 new_window_with_cwd

Then, pressing :kbd:`F1` will open a new window automatically logged into the
same host using the ssh shitten, at the same directory.

The ssh shitten can be configured using the :file:`~/.config/shitty/ssh.conf` file
...
.. code-block:: sh
   ...
   shitty +shitten ssh --shitten interpreter=python servername

The :code:`--shitten` argument can be specified multiple times, with directives
...
this invocation, so any :opt:`hostname <shitten-ssh.hostname>` directives are
ignored.
...
this could be achieved using the ssh shitten with :program:`zsh` and
...
The ssh shitten works by having SSH transmit and execute a POSIX sh (or
:opt:`optionally <shitten-ssh.interpreter>` Python) bootstrap script on the
remote host using an :opt:`interpreter <shitten-ssh.interpreter>`. This script
reads setup data over the TTY device, which shitty sends as a Base64 encoded
compressed tarball. The script extracts it and places the :opt:`files <shitten-ssh.copy>`
and sets the :opt:`environment variables <shitten-ssh.env>` before finally
launching the :opt:`login shell <shitten-ssh.login_shell>` with :opt:`shell
integration <shitten-ssh.shell_integration>` enabled. The data is requested by
the shitten over the TTY with a random one time password. shitty reads the request
and if the password matches a password pre-stored in shared memory on the
localhost by the shitten, the transmission is allowed. If your local
...
.. note::
   ...
   :opt:`interpreter <shitten-ssh.interpreter>` in :file:`ssh.conf`.
...
.. include:: /generated/conf-shitten-ssh.rst
