...
|shitty| has the ability to easily *Edit*, *Open* or *Download* files
...
.. note::
   Nested SSH sessions are not supported. The shitten will always try to copy
   remote files from the first SSH host. This is because there is no way for
   |shitty| to detect and follow a nested SSH session robustly.
