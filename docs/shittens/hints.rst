...
|shitty| has a *hints mode* to select and act on arbitrary text snippets currently
...
to be used can be modified using options passed to the shitten. For example::
...
selected. This allows the hints shitten to provide the user interface, while
...
:file:`custom-hints.py` in the shitty config directory with the following
contents:
