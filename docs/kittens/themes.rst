...
Once that's done, the kitten sends shitty a signal to make it reload its config.
...
usually, :file:`~/.config/shitty/themes`. The kitten will automatically add them
...
it change to that theme instantly. For example::
    ...
    shitty +kitten themes --reload-in=all Dimmed Monokai
