...
.. note::
   This kitten (which practically means shitty) must be installed on the other
...
To send a file from the local computer to the remote computer, simply run::
    ...
    shitty +kitten transfer /path/to/local/file /path/to/destination/on/remote/computer
...
the :option:`--direction <shitty +kitten transfer --direction>` option::

    shitty +kitten transfer --direction=receive /path/to/remote/file /path/to/destination/on/local/computer

Multiple files and even directories can be transferred::

    shitty +kitten transfer file1 dir1 destination/
...
use the :option:`--confirm-paths <shitty +kitten transfer --confirm-paths>`
...
#. When invoking the kitten use the :option:`--permissions-bypass
   <shitty +kitten transfer --permissions-bypass>` to supply the password you set
   in step one.
...
<shitty +kitten transfer --transmit-deltas>` option. Note that this will actually
