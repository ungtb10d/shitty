...
* Make shitty itself available in the remote host :opt:`on demand <kitten-ssh.remote_kitty>`
...
.. code-block:: sh
    ...
    shitty +kitten ssh some-hostname-to-connect-to
...
The ssh kitten can be configured using the :file:`~/.config/shitty/ssh.conf` file
...
.. code-block:: sh
   ...
   shitty +kitten ssh --kitten interpreter=python servername
...
the kitten over the TTY with a random one time password. shitty reads the request
