...
Simply :ref:`install shitty <quickstart>`.  You also need to have either the `git
...
the official shitty binary builds).
...
In the shitty terminal, run::
    ...
    shitty +kitten diff file1 file2
...
.. code-block:: ini
    ...
    [difftool]
        ...
        trustExitCode = true
    [difftool "shitty"]
        cmd = shitty +kitten diff $LOCAL $REMOTE
    [difftool "shitty.gui"]
        cmd = shitty shitty +kitten diff $LOCAL $REMOTE
...
The diff kitten makes use of various features that are :doc:`shitty only
...
:ref:`shitty config folder <confloc>`. See below for the supported configuration
directives.
