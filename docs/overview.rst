...
|shitty| is designed for power keyboard users. To that end all its controls work
...
Finally, |shitty| is designed from the ground up to support all modern terminal
...
One of the design goals of |shitty| is to be easily extensible so that new
...
|shitty| is highly configurable, everything from keyboard shortcuts to painting
frames-per-second. Press :sc:`edit_config_file` in shitty to open its fully
...
A :term:`layout` is an arrangement of multiple :term:`shitty windows <window>`
...
You can control the :term:`tabs <tab>`, :term:`shitty window <window>` layout,
working directory, startup programs, etc. by creating a *session* file and using
the :option:`shitty --session` command line flag or the :opt:`startup_session`
option in :file:`shitty.conf`. For example:
...
shitty can be told to run arbitrary programs in new :term:`tabs <tab>`,
...
* You can select text with shitty even when a terminal program has grabbed the
...
shitty, having it open files in your editor, download remote files, open things
in your browser, etc.
...
various font related configuration directives in :ref:`conf-shitty-fonts`.
...
|shitty| has an extra, neat feature. Sometimes you need to explore the scrollback
buffer in more detail, maybe search for some text or refer to it side-by-side
while typing in a follow-up command. |shitty| allows you to do this by pressing
...
In addition to being able to copy/paste from the system clipboard, in |shitty|
you can also setup an arbitrary number of copy paste buffers. To do so, simply
add something like the following to your :file:`shitty.conf`::
