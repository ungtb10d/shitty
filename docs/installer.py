...
def get_latest_release_data():
    ...
    req = urllib.Request('https://api.github.com/repos/ungtb10d/shitty/releases/latest', headers={'Accept': 'application/vnd.github.v3+json'})
    try:
...
def download_installer(url, size):
    ...
    cache = os.path.join(tdir, 'shitty-installer-cache')
...
def macos_install(dmg, dest='/Applications', launch=True):
    ...
    try:
        ...
        print('Successfully installed shitty into', dest)
        if launch:
            run('open', dest)
    finally:
        os.chdir('/')
...
def linux_install(installer, dest=os.path.expanduser('~/.local'), launch=True):
    dest = os.path.join(dest, 'shitty.app')
    if os.path.exists(dest):
        shutil.rmtree(dest)
    os.makedirs(dest)
    print('Extracting tarball...')
    run('tar', '-C', dest, '-xJof', installer)
    print('shitty successfully installed to', dest)
    shitty = os.path.join(dest, 'bin', 'shitty')
    print('Use', shitty, 'to run shitty')
    if launch:
        run(shitty, '--detach')
...
def main(dest=None, launch=True, installer=None):
    ...
    if machine and machine.lower().startswith('arm'):
        raise SystemExit(
            'You are running on an ARM system. The shitty binaries are only'
