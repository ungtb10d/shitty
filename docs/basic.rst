...
|shitty| is capable of running multiple programs organized into tabs and
...
Additionally, you can define shortcuts in :file:`shitty.conf` to focus neighboring
...
Reload :file:`shitty.conf`           :sc:`reload_config_file` (also :kbd:`^+⌘+f5` on macOS)
Debug :file:`shitty.conf`            :sc:`debug_config` (also :kbd:`⌘+option+f6` on macOS)
Pass current selection to program   :sc:`pass_selection_to_program`
Edit |shitty| config file            :sc:`edit_config_file`
