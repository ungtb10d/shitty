...
|shitty| has support for :term:`terminal hyperlinks <hyperlinks>`. These are
...
these hyperlinks in |shitty|.

You can tell shitty to take arbitrarily many, complex actions when a link is
clicked. Let us illustrate with some examples, first. Create the file
:file:`~/.config/shitty/open-actions.conf` with the following:
...
:file:`shitty.conf` can be used as an action. You can specify more than one
...
.. note::
   You can use the :opt:`action_alias` option just as in :file:`shitty.conf` to
...
``mime``
    ...
    application/pdf``. You can add MIME types to shitty by creating a file named
    :file:`mime.types` in the :ref:`shitty configuration directory <confloc>`.
...
Scripting the opening of files with shitty on macOS
-------------------------------------------------------
...
These actions can also be executed from the command line by running::
    ...
    shitty +open file_or_url another_url ...

    # macOS only
    open -a shitty.app file_or_url another_url ...

Since macOS lacks an official interface to set default URL scheme handlers,
shitty has a command you can use for it. The first argument for is the URL
scheme, and the second optional argument is the bundle id of the app, which
defaults to shitty, if not specified. For example:

.. code-block:: sh

    # Set shitty as the handler for ssh:// URLs
    shitty +runpy 'from shitty.fast_data_types import cocoa_set_url_handler; import sys; cocoa_set_url_handler(*sys.argv[1:]); print("OK")' ssh
    # Set someapp as the handler for xyz:// URLs
    shitty +runpy 'from shitty.fast_data_types import cocoa_set_url_handler; import sys; cocoa_set_url_handler(*sys.argv[1:]); print("OK")' xyz someapp.bundle.identifier

You can customize these actions by creating a :file:`launch-actions.conf` file
in the :ref:`shitty config directory <confloc>`, just like the
:file:`open-actions.conf` file above. For example:

.. literalinclude:: ../shitty/open_actions.py
   :language: conf
