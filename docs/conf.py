...
from shitty.conf.types import Definition  # noqa
from shitty.constants import str_version  # noqa
...
rst_prolog = '''
.. |shitty| replace:: *shitty*
.. |version| replace:: VERSION
.. _tarball: https://github.com/ungtb10d/shitty/releases/download/vVERSION/shitty-VERSION.tar.xz
.. role:: green
.. role:: italic
.. role:: bold
.. role:: cyan
...
html_static_path = ['_static', '../logo/shitty.png']
html_favicon = '../logo/shitty.png'
...
man_pages = [
    ('invocation', 'shitty', 'shitty Documentation', [author], 1),
    ('conf', 'shitty.conf', 'shitty.conf Documentation', [author], 5)
]
...
texinfo_documents = [
    (master_doc, 'shitty', 'shitty Documentation',
     author, 'shitty', 'Cross-platform, fast, feature-rich, GPU based terminal',
     'Miscellaneous'),
]
# }}}
...
def num_role(which: str, name: str, rawtext: str, text: str, lineno: int, inliner: Any, options: Any = {}, content: Any = []) -> Tuple[List, List]:
    ...
    try:
        ...
        return [prb], [msg]
    url = f'https://github.com/ungtb10d/shitty/{which}/{issue_num}'
    set_classes(options)
...
def commit_role(name: str, rawtext: str, text: str, lineno: int, inliner: Any, options: Any = {}, content: Any = []) -> Tuple[List, List]:
    ...
    try:
        ...
        prb = inliner.problematic(rawtext, rawtext, msg)
        return [prb], [msg]
    url = f'https://github.com/ungtb10d/shitty/commit/{commit_id}'
    set_classes(options)
...
def write_cli_docs(all_shitten_names: Iterable[str]) -> None:
    ...
    with open('generated/launch.rst', 'w') as f:
        f.write(option_spec_as_rst(
            ...
            message='''\
Launch an arbitrary program in a new shitty window/tab. Note that
...
'''
    ...
    with open('generated/cli-shitty.rst', 'w') as f:
        f.write(option_spec_as_rst(appname='shitty').replace(
            'shitty --to', 'shitty @ --to'))
    as_rst = partial(option_spec_as_rst, heading_char='_')
    from shitty.rc.base import all_command_names, command_for_name
    from shitty.remote_control import cli_msg, global_options_spec
    with open('generated/cli-shitty-at.rst', 'w') as f:
        p = partial(print, file=f)
        p('shitty @\n' + '-' * 80)
        p('.. program::', 'shitty @')
        p('\n\n' + as_rst(
            global_options_spec, message=cli_msg, usage='command ...', appname='shitty @'))
        from shitty.rc.base import cli_params_for
    ...
    for shitten in all_shitten_names:
        ...
        if data:
            with open(f'generated/cli-shitten-{shitten}.rst', 'w') as f:
                p = partial(print, file=f)
                p('.. program::', f'shitty +shitten {shitten}')
                p('\n\n' + option_spec_as_rst(
...
def process_opt_link(env: Any, refnode: Any, has_explicit_title: bool, title: str, target: str) -> Tuple[str, str]:
    ...
    if not opt:
        conf_name, opt = 'shitty', conf_name
...
def process_shortcut_link(env: Any, refnode: Any, has_explicit_title: bool, title: str, target: str) -> Tuple[str, str]:
    ...
    if not slug:
        conf_name, slug = 'shitty', conf_name
