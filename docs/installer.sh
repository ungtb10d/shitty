...
detect_os() {
    ...
    case "$(command uname)" in
        ...
        'Linux')
            ...
            ;;
        *) die "shitty binaries are not available for $(command uname)"
    esac
}
...
parse_args() {
    ...
    dest="$dest/shitty.app"
}


get_file_url() {
    url="https://github.com/ungtb10d/shitty/releases/download/$1/shitty-$2"
    if [ "$OS" = "macos" ]; then
        url="$url.dmg"
    else
        url="$url-$arch.txz"
...
get_release_url() {
    ...
    [ $? -ne 0 -o -z "$release_version" ] && die "Could not get shitty latest release version"
...
macos_install() {
    tdir=$(command mktemp -d "/tmp/shitty-install-XXXXXXXXXXXX")
    [ "$installer_is_file" != "y" ] && {
        installer="$tdir/shitty.dmg"
        printf '%s\n\n' "Downloading from: $url"
        fetch "$url" > "$installer" || die "Failed to download: $url"
    }
    command mkdir "$tdir/mp"
    command hdiutil attach "$installer" "-mountpoint" "$tdir/mp" || die "Failed to mount shitty.dmg"
    command ditto -v "$tdir/mp/shitty.app" "$dest"
    rc="$?"
...
exec_kitty() {
    if [ "$OS" = "macos" ]; then
        ...
        exec "$dest/bin/shitty" "--detach"
    fi
    die "Failed to launch shitty"
}
