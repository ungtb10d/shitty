...
|shitty| has a :code:`launch` action that can be used to run arbitrary programs
in new windows/tabs. It can be mapped to user defined shortcuts in shitty.conf.
...
In the simplest form, you can use it to open a new shitty window running the
...
piped has a special environment variable declared, ``shitty_PIPE_DATA`` whose
contents are::
...
``@active-shitty-window-id``
    replaced by the id of the currently active shitty window
...
``@input-line-number``
    ...
    scroll position in shitty. See :opt:`scrollback_pager` for details

``@scrolled-by``
    replaced by the number of lines shitty is currently scrolled by
...
For example::
    ...
    map f1 launch my-program @active-shitty-window-id
...
in the window and ``window.id`` is the internal shitty ``id`` of the
window.
