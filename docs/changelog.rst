...
shitty automatically integrates with common shells to allow browsing the
...
- macOS: Allow changing the shitty icon by placing a custom icon in the shitty config folder (:pull:`5464`)
...
- Wayland: Fix for a bug that could cause shitty to become non-responsive when
  using multiple OS windows in a single instance on some compositors (:iss:`5495`)

- Wayland: Fix for a bug preventing shitty from starting on Hyprland when using a non-unit scale (:iss:`5467`)

- Wayland: Generate a XDG_ACTIVATION_TOKEN when opening URLs or running programs in the background via the launch action

- Fix a regression that caused shitty not to restore SIGPIPE after python nukes it when launching children. Affects bash which does not sanitize its signal mask. (:iss:`5500`)
...
- shitty @ set-colors: Fix changing inactive_tab_foreground not working (:iss:`5214`)
...
- A new command :command:`edit-in-shitty` to :ref:`edit_file`
...
- Remote control: Fix commands with large or asynchronous payloads like
  :command:`shitty @ set-backround-image`, :command:`shitty @ set-window-logo`
...
- macOS: Make the global menu shortcut to open shitty website configurable (:pull:`5004`)

- macOS: Add the :opt:`macos_colorspace` option to control what color space colors are rendered in (:iss:`4686`)

- Fix reloading of config not working when :file:`shitty.conf` does not exist when shitty is launched (:iss:`5071`)
...
- macOS: Allow the New shitty Tab/Window Here services to open multiple selected folders.  (:pull:`4848`)
...
- macOS: Allow shitty to handle various URL types. Can be configured via
  :ref:`launch_actions` (:pull:`4618`)

- macOS: Add a new service ``Open with shitty`` to open file types that are not
...
- When dropping URLs/files onto shitty at a shell prompt insert them appropriately quoted and space
  separated (:iss:`4734`)
...
- macOS: Allow opening text files, images and directories with shitty when
  launched using "Open with" in Finder (:iss:`4460`)

- Allow including config files matching glob patterns in :file:`shitty.conf`
  (:iss:`4533`)
...
- Fix showing debug information not working if shitty's :file:`STDIN` is not a tty
  (:iss:`4424`)
...
- Integrate shitty closely with common shells such as zsh, fish and bash.
...
- A new facility :opt:`window_logo_path` to draw an arbitrary PNG image as
  logo in the corner of a shitty window (:pull:`4167`)
...
- A new remote control command to :program:`change the tab color <shitty @
  set-tab-color>` (:iss:`1287`)

- A new remote control command to :program:`visually select a window <shitty @
...
- A new :program:`remote control command <shitty @ set-enabled-layouts>` to change
...
- The command line option ``--watcher`` has been deprecated in favor of the
  :opt:`watcher` option in :file:`shitty.conf`. It has the advantage of
...
- **Backward incompatibility**: No longer turn on the shitty extended keyboard
...
- Fix the :program:`resize OS Windows <shitty @ resize-os-window>` setting a
  slightly incorrect size on high DPI screens (:iss:`4114`)

- :program:`shitty @ launch` - when creating tabs with the ``--match`` option create
...
- macOS: The shitty window can be scrolled by the mouse wheel when OS window not
  in focus. (:pull:`4371`)

- macOS: Light or dark system appearance can be specified in
  :opt:`macos_titlebar_color` and used in shitty themes. (:pull:`4378`)
...
- Fix :option:`shitty --title` not overriding the OS Window title when multiple
...
- macOS: Fix a long standing bug that could cause shitty windows to stop
...
- Fix a regression in the previous release that broke ``shitty --help`` (:iss:`3869`)
...
- When using the OSC 52 escape code to copy to clipboard allow large
  copies (up to 8MB) without needing a shitty specific chunking protocol.
...
- A new remote control command to :program:`resize the OS Window <shitty @
  resize-os-window>`
...
- A new theme for the shitty website with support for dark mode.
...
- Fix reading :option:`shitty --session` from ``STDIN`` not working when the
  :code:`shitty --detach` option is used (:iss:`3523`)
...
- Wayland: Allow using the :code:`shitty --class` command line flag
