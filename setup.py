...
if sys.version_info[:2] < (3, 8):
    raise SystemExit('shitty requires python >= 3.8')
...
verbose = False
build_dir = 'build'
constants = os.path.join('shitty', 'constants.py')
...
def get_python_flags(cflags: List[str], for_main_executable: bool = False) -> List[str]:
    ...
    if fw:
        ...
        if not for_main_executable:
            ...
            # compiling an executable https://github.com/ungtb10d/shitty/issues/289
...
def test_compile(
    ...
    with tempfile.TemporaryDirectory(prefix='shitty-test-compile-') as tdir:
...
def init_env(
    ...
    if native_optimizations and is_macos and is_arm:
        # see https://github.com/ungtb10d/shitty/issues/3126
...
def get_source_specific_defines(env: Env, src: str) -> Tuple[str, Optional[List[str]]]:
    if src == 'shitty/parser_dump.c':
        return 'shitty/parser.c', ['DUMP_COMMANDS']
    if src == 'shitty/data-types.c':
...
def find_c_files() -> Tuple[List[str], List[str]]:
    ...
    for x in sorted(os.listdir(d)):
        ...
        if ext in ('.c', '.m') and os.path.basename(x) not in exclude:
            ans.append(os.path.join('shitty', x))
        elif ext == '.h':
            headers.append(os.path.join('shitty', x))
    ans.append('shitty/parser_dump.c')
    return ans, headers
    ...
    for module in modules.split():
        ...
        if module == 'wayland':
            try:
                ...
                continue
        compile_c_extension(
            genv, f'shitty/glfw-{module}', compilation_database,
...
def compile_shittens(compilation_database: CompilationDatabase) -> None:
    ...
    for shitten, sources, all_headers, dest, includes, libraries in (
        ...
        files(
            ...
            extra_headers=('shitty/charsets.h',),
            extra_sources=('shitty/charsets.c',),
        ...
        compile_c_extension(
            final_env, dest, compilation_database, sources, all_headers + ['shitty/data-types.h'])
...
def build_ref_map() -> str:
    ...
    dest = 'shitty/docs_ref_map_generated.h'
    q = ''
...
def build_launcher(args: Options, launcher_dir: str = '.', bundle_type: str = 'source') -> None:
    ...
    if bundle_type.startswith('macos-'):
        klp = '../Resources/shitty'
    elif bundle_type.startswith('linux-'):
        klp = '../{}/shitty'.format(args.libdir_name.strip('/'))
    elif bundle_type == 'source':
    ...
    objects = []
    for src in ('shitty/launcher/main.c',):
    ...
    dest = os.path.join(launcher_dir, 'shitty')
    desc = f'Linking {emphasis("launcher")} ...'
    cmd = env.cc + ldflags + objects + libs + pylib + ['-o', dest]
    args.compilation_database.add_command(desc, cmd, partial(newer, dest, *objects), key=CompileKey('', 'shitty'))
...
def copy_man_pages(ddir: str) -> None:
    ...
    if not os.path.exists(src):
        raise SystemExit('''\
The shitty man pages are missing. If you are building from git then run:
make && make docs
...
def copy_html_docs(ddir: str) -> None:
    ...
    if not os.path.exists(src):
        raise SystemExit('''\
The shitty html docs are missing. If you are building from git then run:
make && make docs
(needs the sphinx documentation system to be installed)
''')
...
def create_linux_bundle_gunk(ddir: str, libdir_name: str) -> None:
    ...
    for (icdir, ext) in {'256x256': 'png', 'scalable': 'svg'}.items():
        ...
        safe_makedirs(icdir)
        shutil.copy2(f'logo/shitty.{ext}', icdir)
    deskdir = os.path.join(ddir, 'share', 'applications')
    safe_makedirs(deskdir)
    with open(os.path.join(deskdir, 'shitty.desktop'), 'w') as f:
        f.write(
            '''\
[Desktop Entry]
Version=1.0
...
'''
    ...
    with open(os.path.join(deskdir, 'shitty-open.desktop'), 'w') as f:
        f.write(
            '''\
[Desktop Entry]
Version=1.0
Type=Application
Name=shitty URL Launcher
GenericName=Terminal emulator
Comment=Open URLs with shitty
TryExec=shitty
Exec=shitty +open %U
Icon=shitty
Categories=System;TerminalEmulator;
NoDisplay=true
MimeType=image/*;application/x-sh;application/x-shellscript;inode/directory;text/*;x-scheme-handler/shitty;
'''
            )

    base = Path(ddir)
    in_src_launcher = base / (f'{libdir_name}/shitty/shitty/launcher/shitty')
    launcher = base / 'bin/shitty'
...
def macos_info_plist() -> bytes:
    ...
    def access(what: str, verb: str = 'would like to access') -> str:
        return f'A program running inside shitty {verb} {what}'
    ...
    docs = [
        ...
        # Allows dragging arbitrary files to shitty Dock icon, and list shitty in the Open With context menu.
        {
    ...
    url_schemes = [
        ...
        {
            'CFBundleURLName': 'shitty URL',
            'CFBundleURLSchemes': ['shitty'],
            'LSHandlerRank': 'Owner',
    ...
    pl = dict(
        ...
        # Identification
        CFBundleIdentifier=f'net.ungtb10d.{appname}',
        # Bundle Version Info
        ...
        # Localization
        # see https://github.com/ungtb10d/shitty/issues/1233
...
def create_minimal_macos_bundle(args: Options, launcher_dir: str) -> None:
    kapp = os.path.join(launcher_dir, 'shitty.app')
...
def create_macos_bundle_gunk(dest: str) -> None:
    ...
    os.rename(ddir / 'Contents/Frameworks/shitty', ddir / 'Contents/Resources/shitty')
    launcher = ddir / 'Contents/MacOS/shitty'
    in_src_launcher = ddir / 'Contents/Resources/shitty/shitty/launcher/shitty'
    if os.path.exists(in_src_launcher):
        os.remove(in_src_launcher)
    os.makedirs(os.path.dirname(in_src_launcher), exist_ok=True)
    os.symlink(os.path.relpath(launcher, os.path.dirname(in_src_launcher)), in_src_launcher)
...
def package(args: Options, bundle_type: str) -> None:
    ...
    libdir = os.path.join(ddir, args.libdir_name.strip('/'), 'shitty')
    ...
    shutil.copy2('terminfo/shitty.terminfo', os.path.join(libdir, 'terminfo'))
    shutil.copy2('terminfo/shitty.termcap', os.path.join(libdir, 'terminfo'))
    shutil.copy2('__main__.py', libdir)
    shutil.copy2('logo/shitty-128.png', os.path.join(libdir, 'logo'))
    shutil.copy2('logo/shitty.png', os.path.join(libdir, 'logo'))
    ...
    with open(os.path.join(libdir, 'shitty/options/types.py'), 'r+', encoding='utf-8') as f:
        oraw = raw = f.read()
    ...
    def should_be_executable(path: str) -> bool:
        ...
        if len(q) == 2 and q[0] == 'ssh' and q[1] in ('askpass.py', 'shitty'):
            return True
        return False
...
def clean_launcher_dir(launcher_dir: str) -> None:
    for x in glob.glob(os.path.join(launcher_dir, 'shitty*')):
        if os.path.isdir(x):
            shutil.rmtree(x)
        else:
            os.remove(x)
...
def clean() -> None:
    ...
    safe_remove(
        ...
        'linux-package', 'shitty.app', 'asan-launcher',
        'shitty-profile', 'docs/generated')
    clean_launcher_dir('shitty/launcher')
...
def build_dep() -> None:
    ...
    p = argparse.ArgumentParser(prog=f'{sys.argv[0]} build-dep', description='Build dependencies for the shitty binary packages')
    p.add_argument(
        '--platform',
        default='all',
...
def main() -> None:
    ...
    os.chdir(base)
    if args.action == 'test':
        texe = os.path.abspath('shitty/launcher/shitty')
        os.execl(texe, texe, '+launch', 'test.py')
    if args.action == 'clean':
        clean()
        return
    launcher_dir = 'shitty/launcher'
    ...
    with CompilationDatabase(args.incremental) as cdb:
        ...
        elif args.action == 'shitty.app':
            args.prefix = 'shitty.app'
            if os.path.exists(args.prefix):
                shutil.rmtree(args.prefix)
            build(args)
            package(args, bundle_type='macos-package')
            print('shitty.app successfully built!')
        elif args.action == 'export-ci-bundles':
            cmd = [sys.executable, '../bypy', 'export', 'download.calibre-ebook.com:/srv/download/ci/shitty']
