...
if sys.version_info[:2] < (3, 6):
    raise SystemExit('shitty requires python >= 3.6')
...
def init_env(
    ...
    if startup_notification_library is not None:
        ...
        desktop_libs = ['_shitty_STARTUP_NOTIFICATION_LIBRARY="' + startup_notification_library + '"']

    if canberra_library is not None:
        assert('"' not in canberra_library)
        desktop_libs += ['_shitty_CANBERRA_LIBRARY="' + canberra_library + '"']

    if desktop_libs != []:
        library_paths['shitty/desktop.c'] = desktop_libs
...
def get_vcs_rev_defines(env: Env, src: str) -> List[str]:
    ...
    if os.path.exists('.git'):
        ...
        ans.append('shitty_VCS_REV="{}"'.format(rev.strip()))
    return ans
...
SPECIAL_SOURCES: Dict[str, Tuple[str, Union[List[str], Callable[[Env, str], Union[Optional[List[str]], Iterator[str]]]]]] = {
    ...
    'shitty/desktop.c': ('shitty/desktop.c', get_library_defines),
    'shitty/parser_dump.c': ('shitty/parser.c', ['DUMP_COMMANDS']),
    'shitty/data-types.c': ('shitty/data-types.c', get_vcs_rev_defines),
}
    ...
    for module in modules.split():
        ...
        if module == 'wayland':
            try:
                ...
                continue
        compile_c_extension(
            genv, 'shitty/glfw-' + module, compilation_database,
...
def compile_shittens(compilation_database: CompilationDatabase) -> None:
    ...
    def files(
        ...
        return (sources, headers, 'shittens/{}/{}'.format(shitten, output))
    ...
    for sources, all_headers, dest in (
        ...
        compile_c_extension(
            kenv, dest, compilation_database, sources, all_headers + ['shitty/data-types.h'])
...
def build_launcher(args: Options, launcher_dir: str = '.', bundle_type: str = 'source') -> None:
    ...
    if bundle_type.endswith('-freeze'):
        ...
        cppflags.append('-Dshitty_LIB_DIR_NAME="{}"'.format(args.libdir_name))
    ...
    cppflags.append('-Dshitty_LIB_PATH="{}"'.format(klp))
    pylib = get_python_flags(cflags)
    cppflags += shlex.split(os.environ.get('CPPFLAGS', ''))
    cflags += shlex.split(os.environ.get('CFLAGS', ''))
    ldflags = shlex.split(os.environ.get('LDFLAGS', ''))
    ...
    key = CompileKey('launcher.c', 'shitty')
...
def create_linux_bundle_gunk(ddir: str, libdir_name: str) -> None:
    ...
    safe_makedirs(icdir)
    shutil.copy2('logo/shitty.png', icdir)
...
'''
    ...
    base = Path(ddir)
    in_src_launcher = base / (libdir_name + '/shitty/shitty/launcher/shitty')
...
def macos_info_plist() -> bytes:
    ...
    pl = dict(
        ...
        CFBundleIdentifier='net.ungtb10d.' + appname,
        ...
        CFBundleGetInfoString='shitty, an OpenGL based terminal emulator https://sw.ungtb10d.net/shitty/',
...
def create_minimal_macos_bundle(args: Options, where: str) -> None:
    if os.path.exists(where):
        shutil.rmtree(where)
    bin_dir = os.path.join(where, 'shitty.app/Contents/MacOS')
    resources_dir = os.path.join(where, 'shitty.app/Contents/Resources')
    os.makedirs(resources_dir)
    os.makedirs(bin_dir)
    with open(os.path.join(where, 'shitty.app/Contents/Info.plist'), 'wb') as f:
...
def clean() -> None:
    ...
    safe_remove(
        ...
        'shitty-profile', 'shitty/launcher')
...
def option_parser() -> argparse.ArgumentParser:  # {{{
    ...
    p.add_argument(
        ...
        choices='build test linux-package shitty.app linux-freeze macos-freeze build-launcher build-frozen-launcher clean export-ci-bundles'.split(),
...
def main() -> None:
    ...
    with CompilationDatabase(args.incremental) as cdb:
        ...
        if args.action == 'build':
            ...
            dest = ['download.calibre-ebook.com:/srv/download/ci/shitty']
