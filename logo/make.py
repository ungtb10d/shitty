...
unframed_src = os.path.join(base, 'shitty.svg')
framed_src = os.path.join(base, 'shitty-framed.svg')
...
def run(*args):
    try:
        ...
        raise SystemExit('You are missing the {} program needed to generate the shitty logo'.format(args[0]))
...
def main():
    render(abspath('shitty.png'))
    render(abspath('shitty-128.png'), sz=128)
    iconset = abspath('shitty.iconset')
