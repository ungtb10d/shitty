...
Note that you can use the :ref:`shitty @ ls <at-ls>` command to get a list of windows.
'''
MATCH_TAB_OPTION = '''\
--match -m
...
Note that you can use the :ref:`shitty @ ls <at-ls>` command to get a list of tabs.
'''
...
class AsyncResponder:
    ...
    def send_data(self, data: Any) -> None:
        from shitty.remote_control import send_response_to_client
...
def command_for_name(cmd_name: str) -> RemoteCommand:
    ...
    try:
        ...
        raise KeyError(f'Unknown shitty remote control command: {cmd_name}')
