...
def encode_response_for_peer(response: Any) -> bytes:
    return b'\x1bP@shitty-cmd' + json.dumps(response).encode('utf-8') + b'\x1b\\'
...
def handle_cmd(boss: BossType, window: Optional[WindowType], cmd: Dict[str, Any], peer_id: int) -> Union[Dict[str, Any], None, AsyncResponse]:
    ...
    if tuple(v)[:2] > version[:2]:
        if no_response:
            return None
        return {'ok': False, 'error': 'The shitty client you are using to send remote commands is newer than this shitty instance. This is not supported.'}
...
--to
An address for the shitty instance to control. Corresponds to the address
given to the shitty instance via the :option:`shitty --listen-on` option. If not specified,
messages are sent to the controlling terminal for this process, i.e. they
will only work if this process is run within an existing shitty window.


--password
A password to use when contacting shitty. This will cause shitty to ask the user
for permission to perform the specified action, unless the password has been
accepted before or is pre-configured in :file:`shitty.conf`.
...
Used if no :option:`shitty @ --password` is supplied. Defaults to checking for the
:file:`rc-pass` file in the shitty configuration directory.
...
def encode_send(send: Any) -> bytes:
    es = ('@shitty-cmd' + json.dumps(send)).encode('ascii')
...
class SocketIO:
    ...
    def simple_recv(self, timeout: float) -> bytes:
        dcs = re.compile(br'\x1bP@shitty-cmd([^\x1b]+)\x1b\\')
        ...
        if m is None:
            ...
            raise SocketClosed('Remote control connection was closed by shitty without any response being received')
...
cli_msg = (
    ...
    ' :opt:`allow_remote_control` option in :file:`shitty.conf` or use a password, for this'
    ' to work.'
).format(appname=appname)
...
def create_basic_command(name: str, payload: Any = None, no_response: bool = False, is_asynchronous: bool = False) -> Dict[str, Any]:
    ...
    if payload is not None:
        ans['payload'] = payload
    if is_asynchronous:
        from shitty.short_uuid import uuid4
        ans['async'] = uuid4()
    return ans
...
def main(args: List[str]) -> None:
    ...
    if not items:
        from shitty.shell import main as smain
    ...
    try:
        ...
        raise SystemExit(f'Timed out after {response_timeout} seconds waiting for response from shitty')
    except KeyboardInterrupt:
        sys.excepthook = lambda *a: print('Interrupted by user', file=sys.stderr)
        raise
