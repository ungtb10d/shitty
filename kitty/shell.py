...
def print_help(which: Optional[str] = None) -> None:
    if which is None:
        print('Control shitty by sending it commands.')
        print()
...
def run_cmd(
    ...
    try:
        ...
        print_err(f'Timed out after {response_timeout} seconds waiting for response from shitty')
        return
...
def real_main(global_opts: RCOptions, encrypter: CommandEncrypter = NoEncryption()) -> None:
    ...
    pre_prompt = set_window_title('The shitty shell') + set_cursor_shape('bar')
