...
definition = Definition(
    ...
    Action('map', 'parse_map', {'keymap': 'KeyMap', 'sequence_map': 'SequenceMap', 'alias_map': 'AliasMap'},
           ['KeyDefinition', 'shitty.fast_data_types.SingleKey']),
...
if the character is followed by one or more spaces, shitty will use those extra
cells to render the character larger, if the character in the font has a wide
aspect ratio. Using this option you can force shitty to restrict the specified
...
or by defining shortcuts for it in :file:`shitty.conf`, for example::
...
To get the PostScript name for a font, use ``shitty +list-fonts --psnames``:
...
particular, :ref:`shell integration <shell_integration>` in shitty sets the
...
opt('url_prefixes', 'file ftp ftps gemini git gopher http https irc ircs shitty mailto news sftp ssh',
...
to copy to a private shitty buffer. Map a shortcut with the
...
:code:`filter`:
    ...
    the shitty config directory on the pasted text. The text returned by the
...
opt('window_resize_step_cells', '2',
    ...
    long_text='''
The step size (in units of cell width/cell height) to use when resizing shitty
...
relative to the shitty config directory. The logo is displayed in a corner of
every shitty window. The position is controlled by :opt:`window_logo_position`.
...
number of shitty windows in it by window manager (e.g. clicking the window close
...
:code:`custom`
    ...
    :file:`tab_bar.py` in the shitty config directory. For examples of how to
    write such a function, see the functions named :code:`draw_tab_with_*` in
    shitty's source code: :file:`shitty/tab_bar.py`. See also
    :disc:`this discussion <4447>`
    for examples from shitty users.
:code:`hidden`
...
use when editing the shitty config file or similar tasks.
...
set, shitty will run your :opt:`shell` (:code:`$SHELL -l -i -c env`) to see if
your shell startup rc files set :envvar:`VISUAL` or :envvar:`EDITOR`. If that
doesn't work, shitty will cycle through various known editors (:program:`vim`,
...
opt('+remote_control_password', '', option_type='remote_control_password', add_to_default=False,
    long_text='''
Allow other programs to control shitty using passwords. This option can be specified multiple
times to add multiple passwords. If no passwords are present shitty will ask the user for
...
Relative paths are resolved from the shitty configuration directory.
''')
...
shitty window. Relative paths are resolved relative to the shitty config
...
opt('+exe_search_path', '',
    ...
    add_to_default=False,
    long_text='''
Control where shitty finds the programs to run. The default search order is:
...
opt('update_check_interval', '24',
    option_type='float',
    long_text='''
The interval to periodically check if an update to shitty is available (in
...
opt('clipboard_max_size', '64',
    option_type='positive_float',
    long_text='''
The maximum size (in MB) of data from programs running in shitty that will be
...
created. The canonical example is :ref:`clone-in-shitty <clone_shell>`. By
default, shitty will ask for permission for each clone request. Allowing cloning
...
opt('clone_source_strategies', 'venv,conda,env_var,path',
    ...
    long_text='''
Control what shell code is sourced when running :command:`clone-in-shitty`
...
opt('wayland_titlebar_color', 'system',
    ...
    long_text='''
The color of the shitty window's titlebar on Wayland systems with client
...
opt('macos_titlebar_color', 'system',
    ...
    long_text='''
The color of the shitty window's titlebar on macOS. A value of
...
:code:`no`, shitty will use the macOS native :kbd:`Option+Key` to enter Unicode
...
left, right or both :kbd:`Option` keys as :kbd:`Alt`, instead. Note that shitty
itself always treats :kbd:`Option` the same as :kbd:`Alt`. This means you cannot
use this option to configure different shitty shortcuts for :kbd:`Option+Key`
vs. :kbd:`Alt+Key`. Also, any shitty shortcuts using :kbd:`Option/Alt+Key` will
take priority, so that any such key presses will not be passed to terminal
programs running inside shitty. Changing this option by reloading the config is
not supported.
'''
    )
...
opt('macos_hide_from_tasks', 'no',
    option_type='to_bool', ctype='bool',
    long_text='''
Hide the shitty window from running tasks on macOS (:kbd:`⌘+Tab` and the Dock).
Changing this option by reloading the config is not supported.
'''
    )
...
opt('macos_quit_when_last_window_closed', 'no',
    option_type='to_bool', ctype='bool',
    long_text='''
Have shitty quit when all the top-level windows are closed on macOS. By default,
shitty will stay running, even with no open windows, as is the expected behavior
on macOS.
'''
    )
...
are not known as GLFW keys. To see the system key code for a key, start shitty
with the :option:`shitty --debug-input` option, shitty will output some debug text
...
the shitty remote control facility with :option:`launch --allow-remote-control`.
...
If you want to operate on all shitty windows instead of just the current one, use
...
require a full restart of shitty. Particularly, when changing shortcuts for
...
Note that all options from the original :file:`shitty.conf` are discarded, in
...
map('Send arbitrary text on key presses',
    ...
    add_to_default=False,
    long_text='''
You can tell shitty to send arbitrary (UTF-8) encoded text to the client program
...
:code:`application`, :code:`shitty` or a comma separated combination of them.
The modes :code:`normal` and :code:`application` refer to the DECCKM
cursor key mode for terminals, and :code:`shitty` refers to the shitty extended
