...
def ref_hyperlink(x: str, prefix: str = '') -> str:
    t, q = text_and_target(x)
    url = f'shitty+doc://{hostname()}/#ref={prefix}{q}'
...
For comprehensive documentation for shitty, please see: {url}''').format(
...
inside shitty, permanently fixing the OS window's title. So only use this if you
...
to the shitty configuration directory.
...
:italic:`{appname} @` within a shitty window, there is no need to specify the
